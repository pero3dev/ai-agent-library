...
flowchart TD
    ...
    JUDGE -- "正常終了の最終応答" --> DONE["正常完了"]
    JUDGE -- "生成上限・拒否・不正応答" --> STOP
    JUDGE -- "継続可能な停止" --> LIMIT
```
...
3. **応答の解釈** — 本文と停止理由を合わせて調べます。ツール要求、正常終了の最終応答、生成上限による打ち切り、拒否、継続可能な停止を区別します。ツール要求がないことだけでは正常完了と判定できません
...
| 停止条件 | トリガー | 終了時にやること |
| --- | --- | --- |
| 正常完了 | API の正常終了理由と有効な最終応答を確認できる | 成果物を返す。業務上の成功条件は別途検証する |
| 上限到達 | 最大ステップ数 / タイムアウト / コスト上限 / 出力・コンテキスト上限 | 途中経過と未完了理由を報告する。不完全なツール要求は実行しない |
...
拒否応答も、HTTP リクエスト自体は成功していてもタスクの正常完了とは区別します。継続可能な停止は履歴を維持して再開できますが、再開回数にも上限を適用します。停止理由の値と継続方法は採用 API の契約へ対応づけます。
...
def run_agent(task: str) -> str:
    ...
    for step in range(MAX_STEPS):
        ...
        state = classify_stop_reason(response)  # API 固有の停止理由を共通の状態へ変換
        if state == "complete" and valid_final_response(response):
            return response.text
        if state == "continue":
            continue                        # 再開も MAX_STEPS の範囲内
        if state != "tool_calls" or not response.tool_calls:
            raise AgentIncomplete(state, response.text)  # 途中出力を保持。成功として返さない
...
- **ツール要求がない応答をすべて成功とする** → トークン上限による途中出力や拒否を完成品として返す → 停止理由と最終応答の妥当性を確認する
- **ツールエラーで例外を投げてループごと落とす** → モデルに回復のチャンスがなく、1 回の一時的な失敗でタスク全体が失敗する → エラーを観測として履歴に返す
- **ループの中身を記録しない** → 「たまに変な動きをする」を再現も分析もできない → 各イテレーションの入出力・ツール結果を記録する([可観測性とトレーシング](../05-operations/observability-and-tracing.md))

### チェックリスト

- [ ] 最大ステップ数・タイムアウト・コスト上限がコード側で設定されている
- [ ] 正常終了・生成打ち切り・拒否・継続可能な停止を区別している
...
- [Handling stop reasons(Anthropic)](https://platform.claude.com/docs/en/build-with-claude/handling-stop-reasons) — 正常終了、生成上限、拒否、継続可能な停止の具体的な API 契約(アクセス日: 2026-09-10)
