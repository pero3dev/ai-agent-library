...
動画生成(テキスト → 動画)は進歩が速い一方、**現在地を冷静に読む**ことが要ります(2026-08 時点)。
...
- **商用利用・権利は各社規約で確認**: 生成動画の商用利用可否・権利帰属は、各社の利用規約で異なります(2026-08 時点で条件が分岐)。断定せず、使うサービスの規約と法務、規制の入口([業界別規制マップ](../09-business/industry-regulations-map.md))で確認します
- **来歴・透かし**: 生成動画に来歴情報(C2PA)や電子透かしを付与する動きが広がっています(2026-08 時点で主要各社は既定付与の方向)。生成物であることを示す責任を設計に含めます([画像生成のプロダクト組み込み](image-generation-integration.md)の来歴と同じ論点)
