...
- **MDX ガード**: 生成 MDX を再パースし、`TodoCallout` / `PracticeSection` / `GlossaryTerm` / `AttentionWalkthrough` / `AttentionStep` / `ReadingWalkthrough` / `TransformerWalkthrough` / `AttentionVariantsWalkthrough` / `MoEWalkthrough` / `FoundationsWalkthrough` / `ReadingStep`
