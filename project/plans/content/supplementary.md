...
本書は記事追加時に採択した計画を保持しています。以下の現状分析・未着手の表現は作成時点のものです。完了状態と対応フェーズは [計画索引](../../README.md#完了した記事拡張計画)、成果物と以後の保守は [ROADMAP.md](../../../ROADMAP.md) を参照してください。

> **ステータス: ✅ 完了(2026-07-07 に Phase J〜L を完了 = 新規 9 本 + 新セクション 1。L-2 の 2 本は縮小決定により収録せず)。** 進捗の正本は [ROADMAP.md](../../../ROADMAP.md)、以後の変更は定期メンテナンスで管理します。
> [expansion.md](expansion.md) §4 で「検討のうえ除外」とした 5 領域を、**除外理由に対処したうえで収録する**ための別冊計画です。
...
- **接続先**: [ガードレール](../../../docs/06-security/guardrails.md)(指示は「お願い」である理由)、[ファインチューニングと蒸留](../../../docs/03-implementation/fine-tuning-and-distillation.md)(前提知識)
...
- **モデルアーキテクチャの研究動向解説**(論文追跡)— 鮮度管理コストが便益を上回る(→ その後 2026-07-07 のユーザー要望により、安定した学術基礎に限定して収録する計画を [llm-internals.md](llm-internals.md) として別途作成)
...
完了時の規模: EXPANSION 全量(83 本)と合わせて **83 → 92 本**(L-2 の 2 本は縮小決定により収録せず。新セクション計 2)。
...
**決定済み**(2026-07-07、Phase J 着手時):

1. **着手順**: EXPANSION-PLAN の全フェーズ(D〜I)完了後に Phase J から着手(推奨どおり)
2. **新セクション名と番号**: `10-llm-foundations` で確定(EXPANSION で 09-business が採用済みのため)

**決定済み**(2026-07-07、Phase K 着手時):

1. **Q-R3(会話データの規制要件)は独立調査しない**: EXPANSION の P-R5 調査メモ(`research/professional/compliance.md`)と `compliance-and-governance.md` を規制面の正本として参照する。K-2(conversation-data-management)は規制の内容を扱わず、実装側のライフサイクル設計に徹するため

**決定済み**(2026-07-07、Phase L 着手時。ユーザー確認済み):

1. **L-2(金融・医療の考慮点 2 本)は縮小して収録しない**: 推奨どおり「業界規制の入口マップ 1 本まで」とする(ドメイン有識者レビューを完了条件にできないため)。需要が確認できたら独立タスクとして再検討する
2. **フィジカル AI は概観 1 本で確定**: 需要があれば将来の独立タスクで拡張する

## 9. TODO

> **TODO(要確認):** Q-R1〜Q-R3 の調査は各フェーズ着手時に実施する(本計画作成時点では未調査。Phase J は調査不要)(最終確認: 2026-07)
