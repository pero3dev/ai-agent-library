...
ディープリサーチ型エージェントは、調査計画・反復検索・統合をまとめた設計です。RAG 自体も多段検索を含みうるため、検索回数だけで RAG と Agent を区別するものではありません。**曖昧な問い**を受け取り、自分で調査計画を立て、検索と読解を何度も繰り返し、複数の情報源を突き合わせて、**出典付きのレポート**を生成します。人間のリサーチャーの仕事を模す、と言えます。
...
このライブラリ自身の `research/` ディレクトリ(執筆前の公式情報調査メモ)は、この型の記録形式を検討する参考です。「事実 / 出典 URL / 確認日 / 確度」を対応づける形式は、そのままエージェントの出力設計の参考になります。
...
- **確度を表現する**: 一次性、資料の独立性、対象・時点の一致、反証の有無を記録し、**根拠の強さのラベル**を主張ごとに付けます。転載された複数記事は独立した裏付けになりません。このラベルは編集上の判断であり、正答確率として[較正](../04-evaluation/confidence-and-calibration.md)された信頼度ではありません
- **発行主体と日付で重み付けする**: 相反したとき、発行主体(公式かどうか)と情報の鮮度(いつの時点か)で優先順位の手がかりを示します。ただし**最終判断は読者に委ねる**書き方にし、エージェントが独断で「Y は誤り」と断じないようにします

### 社内資料と Web の統合

実務のディープリサーチは、**社内資料と Web**を統合します。所在だけでは内容の正確さや命令としての権限を判断できません。検索時の権限と外部送信経路を分けて管理します。
...
- **主張ごとに根拠を評価する**: 社内制度は承認済み社内規程、外部製品の仕様は提供元の公式資料、というように主張への適合性・発行者・日付を確認します。社内資料にも古い情報や攻撃者由来の指示が混ざりえます。検索文書は所在を問わず、命令ではなく資料として扱います
...
- [ ] 社内資料と Web の検索経路・権限を管理し、根拠の強さは主張への適合性で評価しているか
...
- [Shao ほか, STORM (2024)](https://arxiv.org/abs/2402.14207) — 調査、アウトライン、出典付き記事生成を分ける実装研究(アクセス日: 2026-09-10)
- [Greshake ほか, Indirect Prompt Injection (2023)](https://arxiv.org/abs/2302.12173) — 検索で取り込む資料を通じた間接インジェクション(アクセス日: 2026-09-10)
