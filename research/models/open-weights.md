...
docs は本観測の結果を反映済み(llm-landscape、`last_updated: 2026-08-18`)。確認日 2026-08-18。今回の観測は Llama / Qwen / DeepSeek / Kimi のみで、**Mistral / gpt-oss は未調査**(前回 2026-07-06 の記述のまま)。

1. **Qwen3.8 系の重みが Hugging Face で公開**: `Qwen3.8-27B` や **`Qwen3.8-2.4T-A95B`(総 2.4T / アクティブ 95B 級)** などが HF Qwen org に公開され、**2.4T 級までオープン化**(HF org での公開を確認)。これにより §俯瞰の論点 4「最上位だけクローズド」の Qwen 例(3.7 Max は API のみ)は**不成立**となり、docs 側は「最上位までオープンかはファミリーごと・世代ごとに分かれ、世代交代で変わる」という一般論に書き換え済み
2. **Qwen3.8 のライセンスとコンテキスト長は未確認**: 3.6 までの「All our open-weight models are licensed under Apache 2.0.」が 3.8 にも適用されるかはモデルカード未確認。ctx も未確認(下記 TODO)
3. **Llama 4 の停滞は継続**(2025-04 から約 15 か月、新世代の公開なし)
4. **DeepSeek V4(Pro / Flash・MIT・1M ctx)は現行のまま継続**
5. **Moonshot Kimi K3 は二次情報のみ**: K3 登場の言及は二次情報で見られるが、HF org / 公式一次情報で確認できず(**確認不能**。下記 TODO)
