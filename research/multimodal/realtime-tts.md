# リアルタイム視覚入力(マルチモーダル)+ 単体 TTS の提供状況(2026-08-18 時点)調査メモ
...
- **注意**: リアルタイム API と TTS の提供状況(モデル名・対応言語・上限値・音声クローンの条件)は変化が速い領域です。本メモは **2026-08-18 時点のスナップショット**(初版 2026-07-08)であり、記事執筆時は必ず各社公式で再確認してください(§「変わりやすい項目」参照)
- **確度マーカー(4 段階)**:
...
| --- | --- | --- | --- | --- | --- |
| **Google Gemini Live API** | あり(公式が「low-latency, real-time voice and **vision** interactions」と明記)。ただし連続動画ではなく**画像フレーム列** | 「Video frames are sent as individual images (e.g., JPEG or PNG) at a specific frame rate (**max 1 frame per second**)」。`send_realtime_input` に `video=types.Blob(data=frame, mime_type="image/jpeg")` の形で送る。カメラ・画面いずれもフレーム画像化して送る点は同じ。対応モデルは 2026-08-18 時点で `gemini-3.1-flash-live-preview` / `gemini-2.5-flash-native-audio-preview-12-2025`(映像入力の仕様自体は変更なし) | https://ai.google.dev/gemini-api/docs/live-api/capabilities | 2026-07-08 / 2026-08-18 | 公式確認済み |
| **OpenAI Realtime API** | **native な連続映像入力は「なし」**。離散的な画像(写真・スクリーンショット)を会話に差し込む形は「あり」 | 画像は `conversation.item.create` のユーザメッセージ content part として `type: "input_image"`, `image_url: "data:image/{format};base64,..."` で追加。アプリが「どの画像をいつ見せるか」を制御する設計。`gpt-realtime` / `gpt-realtime-2` / `gpt-realtime-2.1` 系が画像入力に対応(`gpt-realtime-2.1` は 2026-08-18 に追記。仕様自体は変更なし) | https://developers.openai.com/api/docs/guides/realtime-conversations | 2026-07-08 / 2026-08-18 | 公式確認済み |
...
- 「リアルタイム動画をそのままストリーム」できる汎用 API は 2026-08 時点では確認できません。Gemini Live ですら**フレームを静止画に落として最大 1 fps** で送る方式です。記事では「カメラ・画面の"映像"入力 = 実際には低頻度の静止画フレーム列」という設計上の実態を明示するのが安全です。
...
| --- | --- | --- | --- | --- | --- | --- |
| **OpenAI TTS** | REST API(`/audio/speech`)。マネージド API のみ | あり(chunk transfer encoding によるリアルタイムストリーミング。最速化には `wav` / `pcm` 推奨) | `gpt-4o-mini-tts`(最新推奨)、`tts-1`(低レイテンシ)、`tts-1-hd`(高品質)。2026-08-18 の再確認で変更なし | https://developers.openai.com/api/docs/guides/text-to-speech | 2026-07-08 / 2026-08-18 | 公式確認済み |
| **Google Cloud Text-to-Speech** | GCP マネージド API。REST / gRPC | あり(Chirp 3: HD voices が `streaming_synthesize` によるストリーミング TTS に対応、GA) | Chirp 3: HD voices(GA、**全 28 ボイス**。初版の「8 speakers × 31 locales」という記載を修正)、**Gemini-TTS(新モデル群)が登場**(2026-08-18 確認)、従来の Neural2 / WaveNet / Standard も併存 | https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd | 2026-07-08 / 2026-08-18 | 公式確認済み(2026-08-18 再確認) |
...
| --- | --- | --- | --- | --- | --- |
| **OpenAI TTS** | **SSML は非対応**(公式ガイドに SSML の記載なし)。代わりに `instructions` による自然言語のスタイル指示 | 明示的な発音辞書 / phoneme タグの記載を確認できず。固有名詞の読み固定は苦手な設計とみられる(未確認)。SSML 非対応・`instructions` 制御は 2026-08-18 の再確認で変更なし | https://developers.openai.com/api/docs/guides/text-to-speech | 2026-07-08 / 2026-08-18 | 公式確認済み(SSML 非記載)/ 未確認(発音辞書の有無) |
| **Google Cloud TTS** | あり。Chirp 3: HD は SSML 入力に対応(サポートタグ: `<phoneme>`, `<p>`, `<s>`, `<sub>`, `<say-as>`)。ただし**ストリーミング要求では SSML タグ非対応**という制約あり(2026-08-18 の再確認で変更なし)。従来エンジンはより広い SSML | `<phoneme>`(音素指定)、`<sub>`(別語への読み替え=固有名詞対策)、`<say-as>`(数値・日付等の読み方)をサポート | https://docs.cloud.google.com/text-to-speech/docs/chirp3-hd | 2026-07-08 / 2026-08-18 | 公式確認済み(2026-08-18 再確認) |
...
| --- | --- | --- | --- | --- | --- |
| **Google Cloud TTS — Chirp 3: Instant Custom Voice** | あり(高品質録音から短時間でパーソナル音声を生成) | **同意文言の録音提出が必須**。指定スクリプト「I am the owner of this voice and I consent to Google using this voice to create a synthetic voice model.」(言語別テンプレあり、カスタム文言不可)。さらに **allowlist 制**(利用は許可リスト登録ユーザ限定、営業経由で申請)。作成キー上限あり(1 プロジェクト毎分 10 キー)。2026-08-18 の再確認で変更なし(指定同意文言 + allowlist) | https://docs.cloud.google.com/text-to-speech/docs/chirp3-instant-custom-voice | 2026-07-08 / 2026-08-18 | 公式確認済み |
| **Microsoft Azure — Custom Voice(Custom Neural Voice)** | あり(音声サンプルからカスタム neural voice を作成) | **アクセスは limited access(適格性・利用審査あり、intake フォームで申請)**。**voice talent の同意文言録音の提出が必須**(合意契約 + 口頭同意)。consent 文言は多言語(日本語含む)。voice talent 向け開示ガイドライン・Responsible AI ドキュメント群あり。**2026-08-18 の再確認で変更なし(limited access + 同意録音)。ドキュメントのページ名は「Custom voice overview」に変更** | https://learn.microsoft.com/en-us/azure/ai-services/speech-service/custom-neural-voice / https://learn.microsoft.com/en-us/azure/ai-services/speech-service/professional-voice-create-consent | 2026-07-08 / 2026-08-18 | 公式確認済み |
| **ElevenLabs — Professional Voice Cloning(PVC)** | あり(30 分〜3 時間の音声で高品質クローン) | **原則「自分自身の声のみクローン可(we only allow you to clone your own voice)」**。fine-tuning 申請前に **voice verification(voice-captcha による本人確認)** が必須。利用に consent の確認あり。法的可否は Terms of Service / AI Safety 情報を参照するよう案内。2026-08-18 の再確認で変更なし(本人のみ + verification) | https://elevenlabs.io/docs/eleven-creative/voices/voice-cloning/professional-voice-cloning | 2026-07-08 / 2026-08-18 | 公式確認済み(本人のみ・verification)/ 二次情報(captcha の詳細) |
...
| **OpenAI — カスタムボイス** | あり(Custom voices を作成し TTS / Realtime / Chat Completions で利用可) | **公式ガイドに consent recording(指定フレーズの録音提出)が必須との記載を確認(2026-08-18)**。商用条件等の細目は OpenAI の利用規約側で要確認 | https://developers.openai.com/api/docs/guides/text-to-speech | 2026-07-08 / 2026-08-18 | 公式確認済み(consent recording 必須) |
...
1. **Gemini Live の映像入力仕様**: 「最大 1 fps・JPEG/PNG フレーム」「音声+動画セッション 2 分」は preview 段階の制約であり、緩和・変更されうる。**2026-08-18 の再確認で「最大 1 fps のフレーム列」に変更なし**を確認。対応モデルは `gemini-3.1-flash-live-preview` / `gemini-2.5-flash-native-audio-preview-12-2025` に更新。
   > **TODO(要確認):** Gemini Live API の映像フレームレート上限とセッション上限を公式で再確認する(https://ai.google.dev/gemini-api/docs/live-api/capabilities 、最終確認: 2026-08)
2. **OpenAI Realtime の映像対応**: 現状は「離散画像の差し込み」。将来 native 連続映像入力が追加される可能性があるため、`input_image` 以外の映像入力タイプの有無を再確認する。**2026-08-18 の再確認で「native 連続映像なし・`input_image` の差し込みのみ」に変更なし**を確認(`gpt-realtime-2.1` を追記)。
   > **TODO(要確認):** OpenAI Realtime API に native なライブ映像入力が追加されていないか公式ガイドで確認する(https://developers.openai.com/api/docs/guides/realtime-conversations 、最終確認: 2026-08)
3. **TTS のモデル名・エンジン世代**: OpenAI(`gpt-4o-mini-tts` 等)、Google(Chirp 3: HD。2026-08-18 に **Gemini-TTS** という新モデル群の登場を確認)、ElevenLabs(`eleven_v3` 等)はいずれも世代更新が速い。日本語ボイスの追加(特に Polly の Generative/Long-form 日本語対応、Chirp 3 の日本語話者数)は変動しうる。
...
7. **アクセスできなかった/未精読のソース**: OpenAI の gpt-realtime 発表ブログ(openai.com/index/introducing-gpt-realtime/)は voice-agents.md 調査時に 403 で本文取得不可(画像入力の追加は developers.openai.com の realtime-conversations ガイドで裏取り済み)。Amazon Polly Brand Voice の consent 具体要件は未取得のまま(Amazon Polly と料金/課金単位は 2026-08-18 の定点観測では未確認 = TODO 継続)。OpenAI カスタムボイスの consent 要件は 2026-08-18 に公式ガイドで確認済み(consent recording〔指定フレーズの録音提出〕必須)。
