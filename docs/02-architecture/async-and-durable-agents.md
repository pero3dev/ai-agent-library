...
- **何を保存するか**: 完了したステップと結果(特にツール実行の結果)、会話・推論の状態(継続に必要な履歴、または明示的に要約して再開するための要約 + 決定事項)、中間成果物の参照(本体は外部ストレージ)、次にやることの計画
- **どこで切るか**: 意味のある単位で切ります。基本は「ツール実行の完了ごと」と「フェーズ(計画 → 実行 → 検証)の境界」です。細かすぎるチェックポイントは書き込みコストとレイテンシを増やし、粗すぎると再開時のやり直しが増えます
- **再開の設計**: 継続中の会話を再開する場合は、API が要求する履歴・思考ブロック・未完了ツールを保持します。明示的に会話を圧縮して新しく再開する場合は「チェックポイントの構造化状態 + 経緯の要約」から再構築します。要約によるコスト削減だけでなく、キャッシュ再利用とプロトコル上の継続条件も比較して選びます
...
- **受信側が冪等キーに対応する場合**: 同じ業務操作の再試行には同じキーを使います。受信側がキーの記録と副作用を一体として扱い、重複時には同じ操作の結果を返す契約を確認します。キーの有効期間・スコープ・同じキーで異なる引数を送った場合の扱いも必要です([AWS の解説](https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/))
- **自分の DB 内で完結する場合**: キーの一意制約による重複排除と業務データの更新を、同じトランザクションで確定します。「未実行かを読む → 更新する」を分離すると、2 ワーカーが両方とも未実行と判断できます
- **外部 API が冪等キーに非対応の場合**: 外部での成功後・ローカルの完了記録前に落ちると、結果が不明になります。事前チェックやローカルのロックだけではこの隙間を閉じられません。業務 ID による結果照合を行い、成否を確定できるまでは自動再送を止めて人間へ引き継ぎます([エラー処理・リトライ設計](error-handling-and-retries.md))
- **承認イベントの重複排除**: 承認・差し戻しにも一意 ID を持たせ、処理済み ID の記録と状態遷移を同じトランザクションで行います。イベントの重複排除と、承認後に呼ぶ外部 API の冪等性は別に設計します

キーはジョブ ID + ステップ ID などから作れますが、再試行ごとに作り直してはいけません。ジョブを作り直す再受付でも同じ業務操作なら同じキーへ対応させ、別の操作には新しいキーを発行します。耐久実行エンジンのチェックポイントだけで、任意の外部副作用が 1 回限りになるわけではありません。
...
### 非同期ツール API と耐久実行を分ける

非同期ツール使用(async tool calling)は「モデルが待機中にも別の作業をする」機能です。たとえば 2026-09-10 確認の OpenAI Astra は `async: true` のツールに対応し、アプリは結果を元の `call_id` に返します。これは、ワーカーの再起動後も処理を復元する耐久実行の保証とは別です。

ジョブ側には、呼出し ID、外部操作の冪等キー、引数の版、結果、キャンセル要求、承認の対象を永続化します。指示変更後に古い処理が完了した場合は、新しい計画へ使えるかを判定します。承認待ちに入る前後で対象や引数が変わったなら、以前の承認を流用しません。WebSocket の継続処理を使う場合も、切断時の復旧と外部操作の結果照会を自分たちのジョブ契約で定義します。
...
- [ ] 継続再開と要約再開を区別し、思考ブロック・未完了ツール・キャッシュの契約を壊さずに復元できる
- [ ] 完了済みステップ(LLM 呼び出し含む)の結果が再開時に再利用される
- [ ] 副作用ごとに受信側の冪等契約または同一 DB 内の原子性が確認され、結果不明時の照合・自動再送停止条件がある
- [ ] 同じ業務操作の再試行でキーが維持され、承認イベントの重複排除も原子的に行われる
...
- [Making retries safe with idempotent APIs(AWS Builders' Library)](https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/) — クライアント指定キー、更新とキー記録の原子性、遅延した再試行の扱い(アクセス日: 2026-09-10)
