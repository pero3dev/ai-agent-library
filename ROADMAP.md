...
- **08-coding-agents のツール情報の定点観測**(各ページの「変わりやすい項目」と比較表。製品名・プラン・学習ポリシーの変化が速いため四半期ごと必須。`research/coding-agents/` の調査メモを更新起点にする。直近の注目: gpt-5.4 系の Codex 退役 2026-08-31 / GitHub Spark 終了 2026-08-31 / Claude Code の auto 既定化の後続変化 / レガシー Cascade の実終了確認 / Devin Fusion の preview 後の扱い / help.openai.com・devin.ai の取得不能継続)
- **モデルガイド(model-selection / llm-landscape)+ モデル特化プロンプティング(claude / openai / gemini / cross-model-prompting)の定点観測**(モデル名・価格帯・退役日程・各社の思考制御/構造化出力/prefill などプロンプト仕様。四半期ごと必須。`research/models/` と `research/prompting/` の調査メモを更新起点にする。直近の注目: OpenAI o 系・gpt-5 初代退役 2026-12-11 / OpenAI `v1/prompts` 停止 2026-11-30 / Gemini 3.7 Flash 導入価格終了 2026-12-31 / OpenAI レガシー音声系 9 モデル退役 2027-01-20 / Gemini 2.5 系の終了日再設定の有無〔2026-08 に撤回済み〕/ Gemini `thinking_budget` の帰趨 / Qwen3.8 のライセンス一次確認)
