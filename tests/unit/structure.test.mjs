...
test('storage categories partition bytes without following links or counting nested roots twice', t => {
  ...
  for (const [file, bytes] of [['source.md', 3], ['node_modules/pkg/index.js', 5], ['website/out/index.html', 7], ['.git/objects/object', 11], ['.git/harness-eval/case/log', 13], ['.git/harness-tools/bin/tool', 17], ['local.txt', 19]]) write(`storage/${file}`, 'x'.repeat(bytes))
  ...
  assert.equal(result.totals.bytes, 75)
  assert.equal(result.totals.files, 7)
  assert.equal(result.totals.tracked_paths, 1)
  assert.deepEqual(Object.values(result.groups).map(row => row.bytes), [3, 5, 7, 11, 13, 17, 19])
