...
### 安全な集約(Secure Aggregation)

連合学習などで、個々の参加者の更新を集約者へ直接見せずに合計等を得る仕組みです。集約結果や完成モデルからの推論まで防ぐ保証とは分けて扱います。→ [プライバシー強化技術の概観](docs/06-security/privacy-enhancing-technologies.md)

### 一次情報(Primary Source)

発信源そのものの情報(論文の原典・公式ドキュメント・公式ブログ・規格本文・法令)。解説・まとめ(二次情報)と区別し、根拠は一次情報に置くのが情報リテラシーの基本。二次情報は「存在を知る」までに使い、一次まで遡って確かめる。→ [AI 情報の追い方(一次情報の目利き)](docs/00-overview/research-literacy.md)
...
自動化された判断を人間が過度に信頼し、自分で検証・反証しなくなる傾向。訓練された専門家でも起き、LLM の流暢さ・断定的な口調が引き金を強める。意志でなく、検証をデフォルト化する構造で防ぐ。→ [オートメーションバイアスとスキル退化](docs/15-human-ai/automation-bias-and-deskilling.md)

### オープンウェイト(Open Weights)

モデルの重みが公開され、ダウンロードして自社インフラでホストできる LLM。ライセンス条件はモデルごとに大きく異なり、「オープン = 自由に商用可」ではなく、OSI の「オープンソース AI」定義(OSAID)とも区別される。→ [オープンソース AI エコシステム](docs/03-implementation/open-source-ai-ecosystem.md)(ライセンス類型)/ [主要 LLM の全体像](docs/03-implementation/llm-landscape.md)(モデルの地図)
...
改変・再配布物にも同じライセンス条件の継承を求めるライセンスの性質。AI では、有害用途を禁止する振る舞い制限(OpenRAIL 系)が派生物に伝播する形で現れる。用途制限が付くため OSI 定義の「オープンソース」には該当しない。寛容型(Apache/MIT)との対比で押さえる。→ [オープンソース AI エコシステム](docs/03-implementation/open-source-ai-ecosystem.md)
...
保護単位 1 つ分が異なる隣接データセットについて、出力分布の差を ε・δ で制限するプライバシー保証です。あらゆる個人情報の推論を不可能にするものではなく、保護単位と複数公開のプライバシー予算を定義して使います。→ [プライバシー強化技術の概観](docs/06-security/privacy-enhancing-technologies.md)
...
自動化に任せ続けることで、使われなくなった技能が衰える・そもそも習得されなくなる現象。既存技能の萎縮と、若手の学習機会消失(育成問題)を区別する。意図的な手動練習と育成機会の確保で守る。→ [オートメーションバイアスとスキル退化](docs/15-human-ai/automation-bias-and-deskilling.md)
...
### スパースオートエンコーダ(SAE / Sparse Autoencoder)

モデルの活性を、少数だけが発火する解釈しやすい「特徴」の疎な重ね合わせに分解しようとする解釈可能性の手法。1 ニューロンが複数概念を担う多義性(重ね合わせ仮説)への対処として研究される。有効性・網羅性は研究段階の自己報告。→ [LLM の解釈可能性の基礎](docs/11-llm-internals/interpretability-basics.md)
...
データの所有責任・品質・利用条件・更新を組織で管理する仕組み。RAG や社内知識を使う Agent では、知識源の品質・鮮度・権限が回答を制約するため、カタログ・品質基準・AI 利用可否を整備する。モデル・検索・推論等の品質管理とも組み合わせる。→ [AI のためのデータガバナンス](docs/05-operations/data-governance-for-ai.md)
...
下書きモデルの複数トークンを本命モデルでまとめて検証する推論高速化。貪欲法では一致する接頭辞を採用し、確率的生成では受理確率と棄却時の補正分布を用いる。これらを正しく実装し、同じ生成条件を適用する方式では本命の分布を保てる。速度改善は下書きの受理率と検証費用に依存する。→ [推論の内部機構](docs/11-llm-internals/inference-internals.md)
...
運用中に入力分布・対象環境・モデルの挙動等が想定から変化すること。緩やかな変化と急な変化があり、必ず品質劣化を意味するわけではない。品質指標・入力分布・構成変更を照合し、再評価や対応の必要性を判断する。→ [常駐エージェントのライフサイクル管理](docs/05-operations/long-running-agents.md)
...
既存モデルの重みを自分のデータで追加調整すること。形式・文体・ドメイン特化の挙動に加え事実知識も学習し得るが、正確な再現や容易な更新は保証されない。評価・データ準備・再学習の費用を、プロンプト改善・RAG・モデル変更と比較して選ぶ。→ [ファインチューニングと蒸留](docs/03-implementation/fine-tuning-and-distillation.md)
...
プロンプトの例だけから、重みを変えずにタスクを学ぶように振る舞う現象。理論仮説(暗黙のベイズ推定・学習アルゴリズムの模倣・誘導ヘッド)は決着していない。few-shot の例は正解の列挙より、出力の形式・分布の実演として効く。→ [文脈内学習と記憶の科学](docs/11-llm-internals/in-context-learning-and-memorization.md)

### プライバシー予算(Privacy Budget)

差分プライバシーで、同じ保護対象に対する複数の処理・公開を合成して管理するプライバシー損失の上限です。公開を繰り返す際は、各処理の ε・δ と合成の条件を追跡します。→ [プライバシー強化技術の概観](docs/06-security/privacy-enhancing-technologies.md)
...
同じ操作を複数回実行しても副作用の結果が 1 回と変わらない性質です。事前の実行済み確認だけでは並行実行や記録前クラッシュを防げず、受け側の原子的な重複排除などの実装条件が必要です。→ [非同期・長時間タスクの設計](docs/02-architecture/async-and-durable-agents.md)、[エージェントの API 設計](docs/02-architecture/agent-api-design.md)
...
差分プライバシーの隣接データセットを定義するときに、変更する 1 行・1 人などの単位です。1 行の保証が、その人の全記録をまとめた保証になるとは限りません。→ [プライバシー強化技術の概観](docs/06-security/privacy-enhancing-technologies.md)
...
複数のテナント(顧客・部門)が同一の Agent 基盤を共有する構成です。データ・設定・容量・費用を分けて設計し、認証キーを分けた場合も上流の共有上限と容量の隔離を別に確認します。→ [マルチテナント設計](docs/02-architecture/multi-tenancy-and-isolation.md)
...
画像・図表・PDF を含む知識源で行う RAG です。テキスト化や画像埋め込みによる検索と、取得したページを視覚言語モデルで読む処理を分け、知識源に合わせて組み合わせ・評価します。→ [マルチモーダル RAG](docs/12-multimodal/multimodal-rag.md)
...
モデルの想定用途・非推奨用途・既知の限界・学習データ・評価・ライセンス・ベースモデルなどを記した説明書。ハブでは `README.md` の機械可読メタデータ(`license`・`base_model`)+ 本文として実装される。採用前に必ず読み、`license` と intended/limitations を確認する。データ版はデータシート。→ [オープンソース AI エコシステム](docs/03-implementation/open-source-ai-ecosystem.md)
...
対話型 Agent の評価で「ユーザー役」を務める LLM です。指示と会話で誘導するユーザー役、評価対象 Agent の API・状態を制御する環境、採点する審判役は別であり、発話の制約が必要なら検証処理も設けます。→ [ユーザーシミュレータの設計](docs/04-evaluation/user-simulator-design.md)

### 誘導ヘッド(Induction Head)

「直前に A B が出たら、次に A が来たとき B を続ける」というコピー機構を担う注意ヘッド。文脈内学習の主要メカニズムの候補として提案され、形成時期と ICL 能力の立ち上がりの一致が観察・報告されている(原因の証明ではない)。→ [LLM の解釈可能性の基礎](docs/11-llm-internals/interpretability-basics.md)
...
生データを各所に置いたまま学習し、更新情報を集約する手法です。更新や完成モデルからの情報漏えいは残るため、連合学習だけをプライバシー保証とせず、安全な集約や差分プライバシー等の追加対策を検討します。→ [プライバシー強化技術の概観](docs/06-security/privacy-enhancing-technologies.md)
...
組織が AI マネジメントシステム(AIMS)を PDCA で確立・運用するための要求事項を定めた、世界初の AI マネジメントシステム規格(2023 年)。第三者認証の対象になるが、認証は「AI を統制する仕組みの確認」であって AI の安全・正しさの保証ではない。ISMS(ISO/IEC 27001)と統合しやすい。→ [AI 規格・認証の実務](docs/06-security/ai-standards-and-certification.md)
...
機能やデバイスモデルを絞った軽量な仮想マシン。ゲストとホストのカーネルを分離する VM 境界を使う。モデル生成コードの隔離候補だが、起動性能・権限制限・残る攻撃面はハイパーバイザーと実行構成に依存し、ネットワークや秘密情報の制御も別途必要。→ [コード実行サンドボックスの実装](docs/03-implementation/code-execution-sandboxes.md)
...
投資による純利益を投資額で割った率であり、効果から費用を引いた純便益額とは区別します。比較時は評価期間・分母の範囲を揃え、人のレビュー費用を便益と費用で二重に差し引かないようにします。→ [ROI とビジネスケース](docs/09-business/roi-and-business-case.md)

### SFT(教師ありファインチューニング / Supervised Fine-tuning)

「指示 → 望ましい応答」の模範例で LLM を追加学習させる工程。指示追従・形式・文体を調整し、新しい事実も学習し得るが、正確な再現や容易な更新は保証されない。鮮度・出典・権限が重要な知識は検索との比較・併用を検討する。→ [LLM の学習パイプライン(事前学習から選好調整まで)](docs/10-llm-foundations/llm-training-pipeline.md)

### SLM(Small Language Model / 小型言語モデル)

小型の言語モデル。分類・抽出等の限定タスクで費用や速度の改善候補になるが、能力差はモデル・学習・用途に依存する。小型モデルを先に使うルーティングでは、失敗の検出・上位への昇格率・二重実行の費用と遅延まで含めて評価する。→ [小型言語モデル(SLM)の活用戦略](docs/03-implementation/slm-strategy.md)
...
サンドボックス実行を前提とした可搬なバイナリ形式。ホスト関数や WASI が提供する入出力を、実行環境の設定で制限する。起動時間や隔離の強さはランタイム・構成に依存し、ネイティブ依存をそのまま実行できるとは限らない。→ [コード実行サンドボックスの実装](docs/03-implementation/code-execution-sandboxes.md)

### WASI(WebAssembly System Interface)

WebAssembly のプログラムへファイル等のシステム機能を提供するインターフェース群。使用する版・ランタイム・ホスト設定が、利用できる機能と権限を決める。WASI を使うだけでネットワーク遮断や完全な隔離が保証されるわけではない。→ [コード実行サンドボックスの実装](docs/03-implementation/code-execution-sandboxes.md)
