...
- **モデルガイド・モデル特化プロンプティング** — 起点: `research/models/`・`research/prompting/`。次の確認: Fable 5.1 の保持・思考履歴・beta 制御、Astra の API 制約と `configuration_update`、5.6 以降の TTL・書込課金、Gemini 3.8 の価格と API 別思考制御、3.7 導入価格の 2026-12-31 後、OpenAI GPT-5.4-Cyber の 2026-10-01・o4-mini の 2026-10-23 終了予定、`v1/prompts` の 2026-11-30 終了、2026-12-11 の対象モデル終了。Qwen3.8・Kimi K3・Mistral Medium 3.5 の独自契約は配布物と事業条件ごとに再確認
...
- **規制動向** — 起点: `research/professional/compliance.md`。次の確認: 令和 8 年法律第 56 号の主な施行政令、PPC の規則・ガイドライン議論と正式パブコメ、EU AI Act の高リスク義務・整合規格・既存システムのマーキング猶予、California の主体別 2026/2027 年適用、FTC 政策声明案の最終化と商務省州法リスト、Colorado と DOJ 訴訟
...
- **エージェントベンチマーク** — 起点: `research/professional/benchmarks.md`。次の確認: Terminal-Bench 4.0 の版・harness・費用付き結果、WebArena-Verified の課題と評価器、SWE-bench Pro・HAL・GAIA2・AndroidWorld・BFCL・安全性評価。取得していないランキング数値は記載しない
...
- **輸出規制・環境開示** — 起点: `research/strategy/geopolitics.md`・`green-ai.md`。次の確認: BIS・EU の管理品目と個別輸出要件、CAC の越境データ経路、インド DPDP、AWS Sustainability 後継の算定境界、SCI for AI と基本 SCI の規格上の違い、CSRD / EED の適用範囲。統計は推計年・公表年・算定境界を揃える
