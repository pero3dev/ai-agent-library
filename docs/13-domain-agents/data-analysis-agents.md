...
### Text-to-SQL の設計: モデル・データ・文脈を合わせて評価する

Text-to-SQL の精度には、モデルの能力、データの内容、**スキーマと業務文脈の与え方**が関係します。[BIRD](https://arxiv.org/abs/2305.03111)は実データの値や外部知識、SQL の効率も評価課題として扱っています。どの要因が支配的かは対象タスクで切り分けます。生のテーブル定義をそのまま渡すだけでは、現場の暗黙知(この列は論理削除フラグ、金額は税抜、日付は UTC)を踏み外します。

- **スキーマに意味を添える**: テーブル・列の物理名だけでなく、**業務上の意味・単位・注意点**を説明として渡します。`amount` が税抜か税込か、`status='9'` が何を指すかは、DDL には書かれていません
- **分析用ビューを整える**: 正規化された生テーブルを直接触らせるより、**分析用に整えたビュー**(結合済み・意味の明確な列名・論理削除除外済み)を用意し、そこだけを対象にすると結合や定義を取り違える余地を減らせます。改善量は評価セットで確かめます
- **少数例(few-shot)を効かせる**: 頻出の質問と正しい SQL のペアを例示すると、その組織の言い回し(「先月」の定義、「アクティブユーザー」の定義)に沿います
- **書き込みを構造的に禁じる**: 分析用途では、実行アカウントを**読み取り専用**にし、許可したビュー・列だけを参照できる DB 権限にします。文頭が `SELECT` でも、関数や外部アクセスに副作用がないとは限りません。DB ごとの実行権限も絞り、時間・走査量・返却量に上限を設けます。プロンプトでの禁止に頼らず、経路そのものを塞ぎます([エージェントの認証・認可](../06-security/agent-identity-and-auth.md))

### セマンティックレイヤーで指標定義を共有する
...
- **セマンティックレイヤーがない組織の現実解**: 専用の層がない場合でも、**頻出指標の正しい SQL 断片を用意して部品として渡す**方法を比較できます。改善量は対象の質問で測ります。完全な基盤を待たず、指標定義の局所的な一元化から始めます
...
- **構文エラーは自動回復してよい**: 機密情報を除いた実行エラーをモデルに返し、修正候補を作らせます([ループのフィードバックと検証](../03-implementation/loop-feedback-and-verification.md))。再実行前にも権限・費用・回数制限を適用します
- **意味の誤りは実行成功だけでは検出できない**: 集計の意味が違っても実行は成功するため、**別の検証**が要ります。次節の突き合わせがそれです。「エラーが出ないから正しい」という前提を置かないことが、このドメインの肝です
...
- **集計を再現させる**: 重要な数値は、**別の書き方の SQL**でも求めて照合します(例: 明細合計とサマリテーブルの一致)。ただし同じ誤った業務定義を使えば両方が一致するため、指標定義・対象期間・基準データも独立に確認します。不一致時は更新時刻の差なども含めて原因を調べます
...
- **集計での漏えいに注意する**: 個票が見られなくても、**集計結果から個人が特定**できることがあります(該当者が 1 人の集計など)。最小集計人数などの抑止を検討します。ただし少数セルを隠すだけでは k-匿名性を満たすとは限らず、複数クエリの差分から推測される場合もあります。繰り返し照会を含めた開示リスクを評価します([プライバシー強化技術](../06-security/privacy-enhancing-technologies.md))
...
- **正解つき質問セットを作る**: 代表的な質問と、その**正しい答え(数値)**をペアで用意します。生成 SQL の文字列一致ではなく、**実行結果(集計値)の一致**で採点するのが実務的です。SQL の書き方は複数あって構いません。ただし 1 つのデータ状態での一致だけでは偶然の一致を除けません。NULL・重複・ゼロ件・境界日付を含む複数のテストデータと、業務定義に対するレビューを併用します
...
- **「エラーが出ないから正しい」と扱う** → もっともらしい誤答が素通りする → 業務定義・基準データを独立に確認し、既知値との突合と別書き SQL の照合を組み合わせる
...
- [ ] 重要数値の業務定義・基準データを独立に確認し、既知値・別書き SQL と照合しているか
...
- [ケーススタディ: データ分析 Agent](../07-case-studies/case-study-data-analysis-agent.md) — 本記事の設計を「動くのに信用されない」からの回復として物語化した事例
- [ユースケース発見](../09-business/usecase-discovery.md) — データ分析タスクの向き不向き

## 参考資料

- [Li ほか, BIRD (2023)](https://arxiv.org/abs/2305.03111) — 大規模データベース上の Text-to-SQL と、データ内容・外部知識・効率の評価課題(アクセス日: 2026-09-10)
