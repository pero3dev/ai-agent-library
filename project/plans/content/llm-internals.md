...
1. [expansion.md](expansion.md) §4 で「LLM 内部・学習理論」を利用者視点から外れるとして除外
2. [supplementary.md](supplementary.md) が「**数式ではなく実務の判断に効く直感**」に限定する条件付きで [docs/10-llm-foundations/](../../../docs/10-llm-foundations/) を新設(5 本、完了)。ただし §4 で「モデルアーキテクチャの研究動向解説」「数学的基礎の教材化」は引き続き除外
...
フェーズ記号は、既存の調査タスク接頭辞(EXPANSION の P-R\*、SUPPLEMENTARY の Q-R\*)との紛らわしさを避けて **S〜U** を使います(M〜O は [deep-dive.md](deep-dive.md) が使用)。
...
完了時の規模: **92 → 100 本**(+ 新セクション 1。[deep-dive.md](deep-dive.md) と両方完了なら 107 本)。
