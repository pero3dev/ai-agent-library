...
ローカルコマンドのファイル・ネットワーク境界と、境界を越える操作の承認を分けて設計します。次の表は従来の sandbox 設定です。

| 軸 | 選択肢 |
| --- | --- |
| サンドボックスモード(ファイル・ネットワーク境界) | `read-only` / `workspace-write`(既定。ワークスペース内の読み書き・実行可、外部とネットワークは承認要)/ `danger-full-access` |
| 承認ポリシー(いつ承認要求を出すか) | `untrusted`(安全な読み取りのみ自動)/ `on-request` / `never` |

2026-09-10 の公式文書は、権限プロファイル(permission profiles、beta)も案内しています。

- `default_permissions` で組み込みの `:read-only` / `:workspace`、または `[permissions.<name>]` に定義したプロファイルを選びます。旧 `sandbox_mode` / `sandbox_workspace_write` と併用して設定を合成する仕組みではありません
- 読み込まれる設定や `--sandbox` に旧モードがあれば、通常は旧設定が優先します。管理者の `allowed_permission_profiles` がある場合は例外で、プロファイルが強制されます。導入前に旧設定を除去し、組織の混在バージョンは 0.138.0 以降へ揃えます
- `permissions.<name>.network.enabled` は通信可否、`features.network_proxy` はドメイン規則の強制を制御します。通信だけを有効にしてプロキシを無効のままにすると、プロファイルのドメイン制限は適用されません。この境界はローカルコマンド用で、MCP・コネクタ・ブラウザ・クラウドには各機能の制御が必要です

**Auto-review** は、承認要求の判断を別のレビュアーエージェントに渡します。`on-request` 等で承認要求が生じる場合に働き、既に許可された通常操作を毎回検査する仕組みではありません。`never` ではレビューする要求がなく、full access で境界を外した操作の安全も保証しません。
...
- **クラウドとローカルの権限モデルを同一視する** — ローカルはサンドボックスモード × 承認ポリシー、クラウドは 2 フェーズ(セットアップは通信可、Agent フェーズは既定 off・明示有効化可能)と、境界の引き方が違います。→ 面ごとにセキュリティレビューします
