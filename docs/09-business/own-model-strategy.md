...
プロンプト → RAG → ファインチューニング(FT)の先にある「専用モデルへの投資」— 大規模 FT・継続事前学習(continued pre-training)・共同開発や独自構築 — を、**能力・コスト・組織**の観点で判断できるようになります。「持つ」ことがどの段階を指すのかを分解し、判断軸と見落としやすい継続費用(世代交代への追従コスト)、そして「持たない」戦略の強さと撤退基準までを、経営・調達レベルの意思決定として整理します。
...
「自社モデルを持つべきか」という問いは、しばしば「巨大な独自モデルをゼロから学習するか否か」という極端な二択で語られます。実際には、**モデルへの関与は連続した段階**であり、判断はそのどの段階に踏み込むかの選択です。用途によって中間の段階や「持たない」選択も候補になります。
...
各段階を検討するときは、より軽い手段で要件を満たせるかも比較します。これは投資の比較順序を示す設計案です。機密保持、配備先、レイテンシなどの必須要件により、既製 API が候補にならない場合もあります。

| 段階 | 何をするか | 踏み込む前提条件 | 追従コストの重さ |
| --- | --- | --- | --- |
| 既製 API を使い分ける | ベンダーのモデルをそのまま。ティア混在で最適化 | 必須要件を満たすなら比較の基準にする | 学習負担は軽いが移行・評価費用は必要 |
...
1. **差別化の源泉になるか**: そのモデルの挙動が、競合が容易に真似られない事業価値を生むか。汎用能力の改善はベンダー更新でも得られる場合があるため、独自投資との比較対象に含めます。**汎用能力の向上をベンダーに任せ、自社は差別化に集中する**のが基本方針です
...
4. **退役リスクを引き受けられるか**: ホスト型 FT は提供元の退役条件に従うため移行が必要になる場合があります。保持した重みを自社運用するモデルは、提供元の API 退役だけで使用不能になるわけではありませんが、保守・互換性・安全性の維持を自社で引き受けます。次節の追従コストを、事業として引き受けられるか

### 見落としやすい継続費用: 追従コスト

初期構築費に加え、**世代交代への追従、データ整備、監視などの継続費用**を見積もります。何が最大費用になるかは方式と規模で変わります。

既製モデルの更新で、独自モデルとの品質・費用の比較が変わることがあります。ある時点で「既製 API より優秀」だった自社特化モデルは、**次のベース世代が出た瞬間に、追加学習なしの既製モデルに能力で追い越されることがあります**。追いつくには、新しいベースの上で FT・継続学習を**やり直す**必要があり、これはデータ整備・学習・評価・再デプロイの全工程の繰り返しです([ファインチューニングと蒸留](../03-implementation/fine-tuning-and-distillation.md)の「ベース退役は期限付きの計画タスク」)。
...
flowchart TD
    ...
    subgraph 継続費用["見落としやすい継続費用"]
        R1["移行判断時の<br/>再学習・再評価"]
...
投資判断では、初期費用ではなく**「今後 N 年の初期・運用・必要な移行コスト」の総額**で見積もります。独自モデルによる追加便益が、既製 API 案に対する初期費用・追従費用・運用費用の増分に見合うかを比較します。費用だけでなく、機密保持や配備条件など必須要件を満たすかも判断に含めます。

### 「持たない」ことの強さ

「持たない」— 既製モデルを中立的に使い分ける — は、消極的な選択ではなく**積極的な戦略**です。これは[モデル選定ガイド](../03-implementation/model-selection.md)の「モデル中立・乗り換え自由」の経営版です。

- **ベンダーの更新を選択肢にできる**: 新モデルの研究開発費を直接負担せず採用できますが、移行時の評価・設定変更・回帰対応は必要です。自社は差別化レイヤー(データ・プロンプト・ワークフロー・UX)に集中できます
...
原則として、本記事では **既製利用案を比較対象に置き、独自保有の便益と負担を明示する**ことを提案します。唯一の正解や市場全体の傾向を示すものではありません。
...
- **「自社モデルがあると箔が付く」で内製を始める** → 差別化に寄与しないモデルの世話に人材を取られ、本業の差別化が止まる → 4 つの判断軸(差別化・データ優位・維持組織・退役リスク)で問い、必須要件と追加便益に照らして投資理由を説明する
- **初期構築費だけで投資判断する** → 必要な再学習や継続運用の費用を見落とし、採算を過大評価する → 同じ評価期間について、初期構築・通常運用・必要な移行と再学習の費用を合計する
- **プロンプト/RAG を尽くさずに FT・継続学習へ飛ぶ** → 軽い手段で得られた改善に、重い投資が能力で負ける → より軽い案で満たせない要件を明示してから次の段へ
- **世代交代を計画に入れずに特化モデルを塩漬けにする** → 比較対象の改善や要求変更に気づかず、更新の準備が遅れる → ベース退役の追跡と再学習運用をセットで持つ
- **撤退基準なしで投資を始める** → サンクコストで止められず、見合わない内製を続ける → 「既製に並ばれたら」「追従コストが N 倍を超えたら」を着手前に定義する

### チェックリスト

- [ ] 「持つ」のどの段階(FT / 継続事前学習 / 共同開発・独自構築)を検討しているか明確にした
- [ ] より軽い案(プロンプト・RAG・既製の使い分け)を比較し、採用できない場合は理由を記録した
- [ ] 4 つの判断軸(差別化の源泉・データ優位・維持できる組織・退役リスク)すべてに答えた
- [ ] 今後 N 年の初期構築・通常運用・必要な移行と再学習の総費用を、対抗案と同じ範囲で見積もった
...
- [Sculley ほか, Hidden Technical Debt in Machine Learning Systems (2015)](https://papers.nips.cc/paper_files/paper/2015/hash/86df7dcfd896fcaf2674f757a2463eba-Abstract.html) — 学習コード以外の継続保守負担。本文の投資段階と撤退基準は独自の判断枠組みです(アクセス日: 2026-09-10)
