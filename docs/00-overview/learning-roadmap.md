...
このライブラリを「どの順で読むか」を、自分の目的に合わせて決められるようになります。14 のセクションの役割と依存関係を把握し、読者タイプ別の推奨ルートから自分に合うものを選べる状態がゴールです。
