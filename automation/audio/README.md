...
2. Claude Code の隔離したツールなしの実行で対話台本を生成します。必要な前提は、用語集と公開済みの基礎記事から関連する短い抜粋を添えます。別の実行で原文・補助資料との対応、条件・例外・設計理由・重要な識別子、脱落・追加された断定を検査します。
3. 不合格の指摘を使って台本を修正します。上限に達した記事だけを保留にします。
4. 短い発話に分けてローカルで合成し、検証済みの断片を再利用します。チャプターの境界で最大約 60 分に分け、音量を調整して MP3 にします。単一チャプターが長い場合は発話の境界で分割し、説明を省略しません。
5. デコード、長さ、無音、結合前後の時間を検査します。不合格なら対象パートの断片を確認し、不良断片の再生成または再エンコードを設定の上限まで試します。再生成した時間・チャプター位置・ハッシュを再計算し、それでも直らない記事だけを保留にします。制作中に原文が変更されていたら公開用の完了状態にしません。

台本検査と音声信号検査は、発音や自然さを人間が聴いて確認した記録ではありません。問題に気づいた回は、元記事・エピソード ID とともに確認し、該当箇所を直します。

### 別の Agent が台本を書く場合

Codex が初稿や修正版を用意した場合も、同じ原文・補助資料に対する独立レビューと音声検査を通します。台本を置いただけでレビュー済みにはしません。Claude がレビューだけを行った場合は、共同編集者へ追加しません。修正のために台本の内容を変更した場合に追加します。

実際の編集者は、各 job の `authorship/<台本SHA-256>.json` に原文・台本との対応を保存します。SHA-256 は `script.json` を JSON として読み、`JSON.stringify` した値から計算します。ファイルの空白や改行を含むバイト列のハッシュとは異なります。記録と台本の対応を、レビューの再利用、完成済み音声の再利用、公開直前にも検査します。

運用者が確認済みの編集者を登録するときは、[authorship.mjs](../../scripts/audio/authorship.mjs) の `seedScriptAuthorship(jobDir, { article_path, source_digest, expected_script_sha256, agents })` を利用します。現在の台本が指定のハッシュと一致することを確認してから保存し、既存の記録を上書きしません。`agents` は `['codex']`、`['claude']`、共同編集なら `['codex', 'claude']` です。確認していない編集参加を推測して登録しません。

`authorship/` が未導入の従来の Claude 制作物は、その来歴を引き継ぎます。ディレクトリが存在する job で現在の台本に対応する記録が欠けたり壊れたりした場合は、その記事を保留にします。空のディレクトリもこの対象です。正しい記録と台本を復元し、記録の削除によって検査を回避しません。公開の commit・PR・squash は、実際に公開できた記事の編集者をまとめて記載します。
...
公開待ちPRがmainより古くなった場合は、原文・補助資料を再照合し、通常のmerge commitで追随します。自動マージを解除したうえで、PR本文を旧headで先に揃えてから新headをpushします。同じ新headへ本文更新のイベントも重ねてpolicy検査をキャンセルしないためです。途中から再開するときは、既に一致する本文やpushを繰り返しません。

`HELD_REFRESH_METADATA` は、既知の旧headまたは新headで、PRのタイトル・本文を保存済みjournalと安全に照合できない状態です。未知のheadは別の識別エラーになります。旧実装でpush後に停止した場合や他の編集が入った場合も、本文を上書きせずjournalを保持します。

復旧時はPRと `publication/refresh-pr-<番号>.json` の所有を確認し、自動マージを解除したままhead・CI状態を照合します。本文を戻す必要がある場合は、既存のpolicy実行が終了してから、journalの `title` と `body_file` の予定内容に合わせます。その後 `npm run audio:publish -- --apply --auto-merge` で再開します。別の内容を採用したい場合は候補を再レビューし、journalのハッシュを手で書き換えて検査を通しません。
...
linked worktree でも、制作キュー・認証確認・音声断片は元の Git 共通ディレクトリを参照します。`-SyncMain` は初回と `package.json`・`package-lock.json` の変更時に `npm ci` を実行します。依存の準備が失敗した場合は制作へ進みません。

タスクはユーザーのログイン中に低優先度で実行し、PC を起こしません。既定は 4 時間ごと、およびログオンの 2 分後に、1 回最大 1 記事です。初期制作を増やす場合も Claude の普段の利用を優先して上限を調整します。専用 checkout の root で `npm ci` を済ませてから次を実行します。
