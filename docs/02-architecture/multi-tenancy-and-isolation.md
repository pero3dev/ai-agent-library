...
- **バックプレッシャ(backpressure)のスコープ**: テナント別の超過なら当該テナントから絞り、プロバイダーの共有上限に達した場合は全体の送信量も抑えます。全体上限の内側で、対話の優先度とテナントごとの配分を維持します

**認証・計測の分離と、容量の分離は別です。** API キーやプロジェクトを分けると、資格情報の失効や利用量の集計を分けやすくなります。ただし組織・モデル群などの上位枠を共有する場合、キーを増やしても独立した容量は得られません。2026-09-10 に確認した [OpenAI の制限体系](https://developers.openai.com/api/docs/guides/rate-limits)も、組織・プロジェクト単位の制限とモデル間の共有枠を区別しています。

容量を専用化するなら、契約・設定上の専用スループットや独立したクォータの範囲を確認し、負荷試験で他テナントへの影響を測ります。専用キーだけを理由に全体のレート制御を外さず、共有枠に応じた全体上限とテナント別配分を残します。
...
| 部分専用 | 大口テナントに専用キュー・専用インデックス・専用 API キー。必要なら契約で独立した容量枠も確保 | 運用・認証・計測の境界を分けたい。共有する上位容量への影響は別途制御する |
...
- [ ] キー・プロジェクトを分けても共有する上位枠を確認し、全体上限と独立した容量保証を区別している
...
- [OpenAI: Rate limits](https://developers.openai.com/api/docs/guides/rate-limits) — 組織・プロジェクトの制限とモデル間で共有する枠。キー分割を容量保証と同一視しないための具体例(アクセス日: 2026-09-10)

## TODO・未確認事項

> **TODO(要確認):** 採用する LLM プロバイダーの組織・プロジェクト・モデル単位の共有上限と専用容量の契約条件を、各社公式ドキュメントと利用アカウントの設定で確認する(最終確認: 2026-09)
