...
- **信頼境界としての隔離**: 信頼できないコンテンツ(Web ページ・受信メール・共有文書)の処理を低権限の子に分けると、そこで直接使える権限を制限できます。ただし要約にも攻撃者の指示は残りえます。コンテキストを分けただけで、親が受け取る内容まで信頼済みになるわけではありません(詳細は [プロンプトインジェクション](../06-security/prompt-injection.md)が正本)
- **隔離の代償**: サブエージェントは親の文脈を共有しないため、必要な前提を渡し漏れると的外れな結果を返します。委譲の境界は「独立して完結でき、要点だけ返せばよい」単位に引きます

境界では、子の戻り値を「抽出した事実・出典参照・未確認事項」のような出力契約にし、スキーマ・サイズ・参照先をコード側で検証します。自由記述の要約もデータとして扱い、システム指示やユーザーの承認へ昇格させません。親がその内容を根拠にツールを使うときは、親の権限・送信先制限と実行前承認を独立して適用します。構造化出力は形式を制約するもので、内容の無害性の保証ではありません([OWASP の Agent 向け防御](https://cheatsheetseries.owasp.org/cheatsheets/LLM_Prompt_Injection_Prevention_Cheat_Sheet.html))。
...
- **子の要約を信頼済みの命令として親へ戻す** → 要約に残った間接インジェクションが親の権限で実行される → 子を低権限にし、戻り値をデータとして検証し、親の権限制約・実行前承認を維持する([プロンプトインジェクション](../06-security/prompt-injection.md))
...
- [ ] 非信頼コンテンツを扱う子の権限が制限され、戻り値の出力契約・検証と、親の独立した権限制約・承認がある
