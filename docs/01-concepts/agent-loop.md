...
ツール実行の失敗を例外としてループの外に投げると、モデルは何が起きたかを知る機会を失います。エラーメッセージは**モデルへの観測データ**です。「引数の形式が違う」「対象が見つからない」といった情報を履歴に返せば、モデルは引数を直す・別の手段を試すといった回復行動を取れます。リトライをコード側で行うかモデルに任せるかの使い分けは `error-handling-and-retries.md`(執筆予定)で扱います。
...
- `error-handling-and-retries.md`(執筆予定)— リトライ・フォールバックの設計
