...
1. **プロンプトと注入コンテキストの削減** — 共通ルールを短く保ち、開始時の作業ディレクトリ(cwd)と AGENTS.md の読込範囲を合わせます。Codex は起動時にプロジェクトルートから cwd までの経路を読み、cwd より下は自動探索しません。下位に配置するだけでは開始時の指示に入りません
2. **MCP サーバーの絞り込み** — 接続した MCP はすべてコンテキストを太らせ、制限を消費します。使っていないものは外します
3. **軽量モデルへの切替** — ルーチンタスクは軽量系(gpt-5.6-luna 等)へ(gpt-5.4-mini は ChatGPT 認証の Codex で 2026-08-31 に提供終了。API キー認証と OpenAI API はこの終了の対象外)。サブエージェントにも軽量モデルを割り当てるのが公式の想定です
...
- [ ] 開始 cwd と、ルートからそこまでの AGENTS.md の読込経路を確認したか。経路外の規約が必要なら、対象ディレクトリでの起動または明示的な読込依頼を選び、実際に読まれた指示を確認したか
