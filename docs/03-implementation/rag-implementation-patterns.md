...
flowchart LR
    ...
    subgraph R["2. 検索(オンライン)"]
        Q["クエリ + 実行主体の権限条件"] --> S["権限条件付きの<br/>ベクトル / キーワード /<br/>ハイブリッド検索"] --> RR["リランキング"]
    end
    subgraph G["3. 生成"]
...
- **順序**: 利用者の権限条件を確定 → その条件を検索時に適用して候補取得 → リランキング → 上位 k 件をコンテキストへ、を基本候補にします。権限条件はモデルが省略・変更できないようツール実装側で強制します

件数制限の後で権限フィルタを掛ける方式では、上位候補が権限外に偏ると、下位にある閲覧可能な文書を取りこぼします。基盤の制約などで後フィルタを選ぶ場合は、候補数の拡張・追加取得を設計し、**利用者が閲覧可能な正解文書を母集団とした再現率**を測ります。いずれの方式も、権限外の候補を生成モデルや外部のリランキングサービスへ渡してはいけません。検索基盤によるフィルタ時点の違いは [Azure AI Search の解説](https://learn.microsoft.com/en-us/azure/search/vector-search-filters)に具体例があります。

### Agentic RAG の実装

クエリの言い換え、複数ソースの検索、取得値の計算は、**固定された多段ワークフローでも実装できます**。例えば「クエリ書き換え → 2 つの索引を並列検索 → 統合 → 計算 → 回答」は、各ステップに LLM を含めても経路は事前に定義できます。途中結果に応じてモデルが検索先・再検索・次の操作を動的に選ぶ必要がある場合に Agentic RAG を検討します。操作数だけを採用理由にしません([Anthropic の Workflow / Agent の区別](https://www.anthropic.com/engineering/building-effective-agents)、[RAG と Agent の関係・使い分け](../01-concepts/rag-vs-agent.md))。

検索の制御を Agent に渡すと決めた場合は、次を設計します。
...
- **クエリの再構成を任せる**: 必要に応じて、会話履歴を踏まえた具体化・分解をモデルに行わせます。クエリ書き換え単体は固定ワークフローにも置けるため、その効果と動的な経路選択の効果を分けて評価します
...
- [ ] フィルタの適用時点を確認し、件数制限による取りこぼしを権限制限下の再現率で評価した
- [ ] 固定の多段ワークフローと比較し、Agentic RAG が必要なら動的な経路選択の理由を説明できる
...
- [埋め込み(embeddings)の選定と運用](embeddings.md) — 埋め込みモデルの選定・評価・再インデックス(本記事の埋め込み層の詳解)
- [ベクトルデータベースの選定と運用](vector-databases.md) — 検索基盤(ベクトル DB)の選定・運用
- [LLM 向けデータ前処理パイプライン](data-preprocessing-for-llm.md) — チャンキング以前の抽出・クリーニング・メタデータ設計
- [GraphRAG とナレッジグラフ](graph-rag-and-knowledge-graphs.md) — 関係たどり・全体集約に強いグラフ型の別方式(まず通常の RAG で足りるかを判断)
- [マルチモーダル RAG](../12-multimodal/multimodal-rag.md) — 画像・図表・PDF を含む知識源への拡張(本記事のマルチモーダル差分)
- [ドキュメント AI(帳票・PDF の構造化)](../12-multimodal/document-ai.md) — 文書を検索可能な構造化データに取り込む前段
- [新興攻撃パターンの体系](../06-security/advanced-attack-patterns.md) — 知識源ポイズニング(細工された文書の混入)の防御面
- [ディープリサーチ型エージェントの設計](../13-domain-agents/deep-research-agents.md) — 多段検索・出典管理・レポート生成(本記事の検索基盤を自律調査に組み上げる応用)
- [検索体験の再設計](../13-domain-agents/search-experience-redesign.md) — 本記事の検索基盤を人間向けの回答型検索プロダクトに載せる応用
- [examples/python/rag-basics/](../../examples/python/rag-basics/README.md) — ベクトル DB を使わない純 Python の最小 RAG(検索と根拠付き生成・モック実行可)

## 参考資料

- [Azure AI Search: Vector query filters](https://learn.microsoft.com/en-us/azure/search/vector-search-filters) — 検索前・検索後のフィルタ適用と、候補件数による取りこぼしの関係(アクセス日: 2026-09-10)
- [Building effective agents(Anthropic)](https://www.anthropic.com/engineering/building-effective-agents) — 事前定義のワークフローとモデルが動的に経路を選ぶ Agent の区別(アクセス日: 2026-09-10)
