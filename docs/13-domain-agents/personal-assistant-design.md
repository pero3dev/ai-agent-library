...
したがって設計の中心テーマは「**権限の広さ**」と「**自律度**」を独立に絞り込むことです。便利さのために権限を広げるなら自律度は低く保ち、自律度を上げるなら権限を絞る、という設計上の目安で制御します。定量的なリスク計算式ではなく、扱うデータ・送信先・操作の影響も評価します。この記事の各節は、この 2 軸をどう設計するかに帰着します。

### 横断アクセスの設計(ツール群と権限の最小化)

アシスタントに「全部できる 1 個の万能アカウント」を与えるのは最悪の出発点です。ツールと権限を、**用途ごとに分けて最小化**します。

- **読み取りと書き込みを分ける**: 「メールを読む」と「メールを送る」は別のツール・別の権限にします。多くの有用な機能(要約・検索・下書き)は読み取りだけで実現でき、送信・削除のような不可逆操作を独立に制御できます。読み取り結果の表示・モデル送信・検索クエリにも情報流出対策が必要です
- **スコープを絞る**: カレンダーなら「予定の閲覧」と「予定の作成」、メールなら「特定ラベルのみ」のように、アクセス範囲を業務に必要な最小限に絞ります。提供 API の権限スコープで細分化できない範囲は、ツール側の対象許可リストと認可検査で強制します([エージェントの認証・認可](../06-security/agent-identity-and-auth.md))
- **持ち主の権限で動く**: アシスタントは持ち主本人の権限の範囲でのみ動き、それを超える操作(他人の予定の閲覧など)はできないようにします
- **致命的な組み合わせを避ける**: 「非公開データへのアクセス」「信頼できない外部コンテンツの読み取り」「外部への送信能力」の 3 つが 1 つのエージェントに揃うと、データ持ち出しが構造的に可能になります([Agent の脅威モデル概観](../06-security/threat-model-overview.md)の致命的三重奏)。パーソナルアシスタントはこの 3 つが揃いやすいため、承認に加えて、送信先・内容・権限をツール側で検査します。承認ゲートだけでは外部への送信能力そのものはなくなりません
...
- **① 読み取り**: 要約・検索など、元データを書き換えない機能から有用性を検証します。ただし機密の誤表示や検索クエリ、外部画像の取得などを通じた流出は起こりえます。読み取り専用を無害とは扱いません
- **② 下書き**: 返信案・予定案を**提示するだけ**で、送信・確定はしません。人が確認・修正できる余地を残します。ただし共有フォルダやメールサービスへ下書きを保存する操作は書き込みです。保存先の公開範囲や、不適切な提案による影響も確認します
...
- **確度の低い先回りは黙る**: 「たぶんこうしたいはず」の確度が低いときは、提案せずに黙るほうが信頼されます([信頼度と較正](../04-evaluation/confidence-and-calibration.md)の棄権の考え方)。外れた提案が信頼を損なう可能性を、利用者の反応と評価で確かめます
...
- **送信・持ち出しに人を挟む**: 完全な防御は困難なため、**被害を止める**設計を併用します。外部への送信・転送・共有は、宛先・確定した内容・共有範囲を提示して承認を得ます。承認後の内容差し替えを防ぎ、危険な送信先はツール側でも拒否します。外部画像や URL の取得、検索などの経路も対象にし、人が見落とす場合に備えます([データ漏えい対策](../06-security/data-exfiltration.md))
...
- [ ] 送信・削除・持ち出しに具体的な内容の承認とツール側の権限・送信先検査を適用したか
...
- [Greshake ほか, Indirect Prompt Injection (2023)](https://arxiv.org/abs/2302.12173) — 取得コンテンツ経由の操作誘導・情報漏えいの攻撃面(アクセス日: 2026-09-10)
- [Amershi ほか, Guidelines for Human-AI Interaction (CHI 2019)](https://www.microsoft.com/en-us/research/wp-content/uploads/2019/01/Guidelines-for-Human-AI-Interaction-camera-ready.pdf) — 介入のタイミング、却下・訂正、利用者制御(アクセス日: 2026-09-10)
