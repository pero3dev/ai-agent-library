...
本書は PR #11・#12 の導入・受入時点の記録です。以下の 6 必須チェックや CLI 版は当時の条件を保持しています。後続の 9 必須チェックへの拡張と実 Agent の受入は [ハーネス実施記録](harness-acceptance.md)、操作方法は [現行運用](../../../freshness-automation.md) を参照してください。
