...
本記事が扱う自己回帰型 LLM の基本動作は、**「これまでのトークンを条件に次の 1 トークンの確率分布を計算し、そこから 1 つ選んで末尾に足す」を、停止条件まで繰り返す**ことです。これを自己回帰生成(autoregressive generation)と呼びます。実装ではキャッシュや投機的デコーディングを使う場合もあります。トークン(token)はモデルが扱うテキストの単位です(詳細は[トークナイザとトークン経済](tokenization.md))。
...
**「1 回通ったテスト」だけでは品質を保証できません。** 評価を複数ケース・複数回で設計します([Agent 評価の基礎](../04-evaluation/agent-evaluation-basics.md))。自由文の回帰テストでは意味や要件への適合も評価します。一方、分類ラベル・集計値・必須のツール引数は完全一致を成功条件にでき、その成功率を複数回の実行で測ります([回帰テストと CI 組み込み](../04-evaluation/regression-testing.md))。

### 停止とストリーミング

生成の停止には、例えば次の種類があります。API ではツール要求・拒否・継続可能な停止なども別に表現されるため、採用先の停止理由に対応づけます。

- **自然停止**: モデルが「応答の終わり」を意味するトークンを選ぶ
- **停止シーケンス**: 開発者が指定した文字列が現れたら打ち切る
- **上限打ち切り**: 最大出力トークン数(max_tokens)に達して強制終了する

上限で打ち切られると、JSON の構文が壊れたり、構文は有効でも必要な結果が欠けたりします。停止理由(finish reason)と出力の検証をせず後続へ流すと、不完全な結果を成功扱いしかねません([構造化出力](../03-implementation/structured-output.md)、[Agent ループ](../01-concepts/agent-loop.md))。
...
- **自由文の表現まで温度 0 で固定されると期待する** → 意味が同じ言い換えでも回帰失敗になる → 自由文は意味や要件で評価し、分類ラベル等の一致が必要な出力とは基準を分ける
...
- [Attention Is All You Need](https://arxiv.org/abs/1706.03762) — 自己回帰デコーダーと次トークンの出力分布(アクセス日: 2026-09-10)
- [The Curious Case of Neural Text Degeneration](https://arxiv.org/abs/1904.09751) — デコード方法による生成品質の違いと nucleus sampling(top-p)(アクセス日: 2026-09-10)
- [Handling stop reasons(Anthropic)](https://platform.claude.com/docs/en/build-with-claude/handling-stop-reasons) — 正常終了・上限・拒否・継続の具体的な API 契約(アクセス日: 2026-09-10)
