...
- **調査日:** 2026-07-08(初版)/ 2026-08-18(四半期定点観測で更新)
...
> **TODO(要確認):** 本メモの主要項目は 2026-08-18 の四半期定点観測で再確認済み(Adobe 系はタイムアウト継続、OpenAI 来歴の一次ページは 403 継続のため据え置きあり)。docs 本文へ反映する前に、少なくともモデル名・提供形態・来歴機能の 3 点を各公式ページで再確認する(最終確認: 2026-08)
...
| 代表例(2026-08 実在) | 提供形態の類型 | 機能(公式ドキュメント記載) | 出典(公式) | 確度 |
| --- | --- | --- | --- | --- |
| OpenAI **GPT Image**(最新 `gpt-image-2`。ほか `gpt-image-1.5` / `gpt-image-1` / `gpt-image-1-mini`) | API 提供 | t2i / 画像編集(部分・全体)/ マスクによるインペインティング / 参照画像による生成。エンドポイントは Generations と Edits の 2 系統 | [image-generation guide](https://developers.openai.com/api/docs/guides/image-generation)、[gpt-image-1 model](https://developers.openai.com/api/docs/models/gpt-image-1) | 公式確認済み |
| Google **Imagen**(Gemini API / Vertex AI 経由) | API 提供 | t2i、画像数・サイズ・アスペクト比・人物生成設定の制御。**公式 deprecations 表で `imagen-4.0-generate-001` / `-ultra` / `-fast` の shutdown 日 2026-08-17(到来済み)・移行先 `gemini-3.1-flash-image` を確認(2026-08-18)**。API の実停止挙動(エラー応答)の実測は未実施 | [Imagen docs](https://ai.google.dev/gemini-api/docs/imagen)、[deprecations](https://ai.google.dev/gemini-api/docs/deprecations) | 公式確認済み |
| Google **Nano Banana**(Gemini ネイティブ画像)。2026-08-18 時点で 4 モデル体制: **Nano Banana 2 Lite**(`gemini-3.1-flash-lite-image`)/ **Nano Banana 2**(`gemini-3.1-flash-image`)/ **Nano Banana Pro**(`gemini-3-pro-image`)/ **Nano Banana**(`gemini-2.5-flash-image`) | API 提供(Gemini API / AI Studio / Vertex AI) | t2i / テキスト指示による画像編集(追加・削除・スタイル転写・インペインティング)/ 参照画像合成は最大 14 枚(3.1 Flash Image)。**全生成画像に SynthID 透かし(C2PA の記載なし)** | [image-generation docs](https://ai.google.dev/gemini-api/docs/image-generation) | 公式確認済み |
...
| Black Forest Labs **FLUX**(2026-08-18 時点で **FLUX.2 世代**: `FLUX.2 [dev]` / `[klein]` 4B / `[klein]` 9B。旧 `FLUX.1 [schnell]` / `[dev]` / `Kontext [dev]` も存続。**FLUX 3 への言及あり**) | オープンウェイト(自ホスト可)+ API | t2i、FLUX.1 Kontext は画像編集(参照画像ベース)向け | [BFL Licensing](https://bfl.ai/licensing) | 公式確認済み |
| **Midjourney** | 統合サービス(Web / Discord)。公式の汎用パブリック API は 2026-08 時点でも確認できず | t2i、バリエーション、編集系(公式ドキュメント参照) | [Midjourney ToS](https://docs.midjourney.com/hc/en-us/articles/32083055291277-Terms-of-Service) | 公式確認済み(サービス提供)/ API 有無は `未確認` |
...
> **TODO(要確認):** Midjourney の公式 API 提供状況(パブリック API が一般提供されているか)を [docs.midjourney.com](https://docs.midjourney.com/) で確認する。2026-08-18 の再確認でも公式一次情報は得られず、API への言及は二次情報のみ(最終確認: 2026-08)
...
| Black Forest Labs FLUX | `FLUX.1 [schnell]` は **Apache 2.0**(商用可)。`FLUX.1 [dev]` / `Kontext [dev]` は **非商用ライセンス**。**FLUX.2 世代(2026-08-18 確認)**: `FLUX.2 [dev]` は非商用 / `FLUX.2 [klein]` 4B は **Apache 2.0**(検索スニペット経由・下記 TODO)/ `[klein]` 9B は非商用ライセンス v2.1。商用は API 利用(全用途で商用権付き)か自ホスト用の商用ライセンス(**Builder / Platform / Professional / Enterprise の 4 層**、セルフサーブ購入可)が必要 | 公式確認済み(FLUX.2 [klein] 4B のライセンスのみ検索スニペット経由) |
...
> **TODO(要確認):** FLUX.2 [klein] 4B の Apache 2.0 ライセンスは検索スニペット経由でしか確認できていないため、[BFL Licensing](https://bfl.ai/licensing) の本文で再確認する。FLUX 3 への言及の詳細(提供時期・ライセンス)も併せて確認する(最終確認: 2026-08)
...
| --- | --- | --- | --- | --- |
| OpenAI(DALL·E / GPT Image) | 生成画像に C2PA メタデータを付与。**2026-05-19/20 頃に C2PA へ Conforming Generator として参加**と発表。検証ツール **openai.com/verify** を公開。ヘルプ記事は「Provenance signals (Content Credentials, SynthID) in OpenAI-generated content」に改題 | **SynthID を ChatGPT / API / Codex の全生成画像に統合**と発表 | [help article 8912793](https://help.openai.com/en/articles/8912793-c2pa-and-synthid-in-openai-generated-images)(本文取得は 403 継続) | ベンダー自己報告 / 二次情報(一次ページ 403 のため。2026-08-18 再確認) |
...
主要モデルの類型・提供形態・尺/解像度の一般的制約・音声同期の有無(2026-08 時点。Sora〔`sora-2` / `-pro`・16/20 秒・pro 1080p・音声付き〕と Veo〔`veo-3.1` 系・4/6/8 秒・1080p/4K は 8 秒のみ・ネイティブ音声・SynthID・Preview〕は 2026-08-18 の再確認で変更なし)。数値は各公式ドキュメントの記載で、モデル改定で頻繁に変わる。

| 代表例(2026-08 実在) | 提供形態 | 尺 / 解像度(公式記載) | 音声同期 | 出典(公式) | 確度 |
| --- | --- | --- | --- | --- | --- |
...
| Amazon **Nova Reel**(1.0 / 1.1、Bedrock) | API 提供。**Bedrock モデルカードに「Model lifecycle: Legacy (certain regions)」「Model EOL date: September 30, 2026」の記載を確認(2026-08-18)**。後継として Luma Ray v2 が案内される(二次情報) | Reel 1.0=最大 6 秒、Reel 1.1=最大 120 秒(6 秒ショットの結合、Single/Multi-shot)。入力画像は 1280x720。**英語プロンプトのみ**(仕様は 2026-08-18 時点で変更なし) | **なし**(音声・3D 非対応と明記) | [Nova Reel AI Service Card](https://docs.aws.amazon.com/ai/responsible-ai/nova-reel/overview.html)、Bedrock の Nova Reel モデルカード | 公式確認済み(Legacy / EOL はモデルカード)。後継情報のみ二次情報 |
...
> **TODO(要確認):** Amazon Nova Canvas(画像)側にも同様の Legacy / EOL 設定が入っていないか、Bedrock の Nova Canvas モデルカードで確認する。Nova Reel の後継として案内される Luma Ray v2 の提供形態・商用条件・来歴も一次情報で確認する(最終確認: 2026-08)
...
**Gemini API の動画理解(公式確認済み。2026-08-18 の再確認で既定 1 FPS・トークン構造〔258 / 66 トークン/フレーム・音声 32 トークン/秒〕に変更なし):**
...
> **TODO(要確認):** Gemini 動画理解の fps カスタム指定(`videoMetadata` の `fps`)の可否と入力サイズ上限を公式ドキュメント本文で再確認する。2026-08-18 の取得要約では「File API 20GB / 2GB・インラインデータ <100MB」とあり、本メモ初版の「20MB 超は File API 必須推奨」と不一致(最終確認: 2026-08)
...
| --- | --- | --- | --- |
| OpenAI GPT Image | 付与。**C2PA へ Conforming Generator として参加**と発表(2026-05-19/20 頃)。検証ツール openai.com/verify を公開 | **SynthID を ChatGPT / API / Codex の全生成画像に統合**と発表 | ベンダー自己報告 / 二次情報(一次ページ 403 継続・2026-08-18 再確認) |
...
| 項目 | 変わりやすさ | 再確認先 |
| --- | --- | --- |
| モデル名・世代(gpt-image-*、Veo 3.x、Imagen→Nano Banana など) | 高。**Imagen(imagen-4.0 系)は 2026-08-17 に shutdown 到来済み**の実例あり | 各公式モデルドキュメント |
...
- OpenAI のヘルプ記事(「Provenance signals (Content Credentials, SynthID) in OpenAI-generated content」に改題)本文は 2026-08-18 も 403 継続。C2PA への Conforming Generator 参加(2026-05-19/20 頃)・SynthID の ChatGPT / API / Codex 全生成画像への統合・openai.com/verify 公開は検索スニペット・二次情報での確認に留まるため、docs 反映前にブラウザで本文を直接確認する
- Adobe の Firefly FAQ / Content Credentials overview は WebFetch がタイムアウト(2026-08-18 の再取得もタイムアウトのため据え置き)。検索スニペット(公式ページ由来の引用)で確認したが、docs 反映前に本文を直接確認する
- Midjourney の公式パブリック API 提供状況が `未確認`(2026-08-18 再確認でも二次情報のみ)
- Imagen(imagen-4.0 系)の shutdown(2026-08-17)は公式 deprecations 表で確認済みだが、API の実停止挙動(エラー応答)の実測は未実施
- FLUX.2 [klein] 4B の Apache 2.0 ライセンスは検索スニペット経由(bfl.ai 本文での再確認が必要)。FLUX 3 への言及の詳細も未確認
- Amazon Nova Canvas の Legacy 化有無と、Nova Reel 後継として案内される Luma Ray v2 の一次情報が `未確認`
- Gemini 動画理解の入力サイズ上限(取得要約「File API 20GB / 2GB・インライン <100MB」と初版記載「20MB」の不一致)と fps カスタム可否の本文確認が未了
