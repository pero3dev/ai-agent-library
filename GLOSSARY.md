...
小さく速い下書きモデルで次の数トークンをまとめて予測し、本命モデルで一括検証して合う分だけ採用する推論高速化。出力分布は本命モデルと同一に保たれる(近似ではない)。メモリ帯域律速なデコードを、当たれば 1 回の計算で複数トークン進めて加速する。→ [推論の内部機構](docs/11-llm-internals/inference-internals.md)
