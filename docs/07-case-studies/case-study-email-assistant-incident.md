...
- **封じ込め**: インシデント対応の手順に従い、最初に応答表示の外部画像取得と Agent の外部通信を遮断し、進行中のセッションを停止しました。既存の履歴にも非公開メールと攻撃指示が残るため、社内検索を止めるだけでは不十分です。遮断をログとテストで確認した後、履歴を引き継がない新規セッションで、対象メールだけをプレーンテキストで要約する縮退機能を再開しました([インシデント対応](../05-operations/incident-response.md))。安全な通信制限を確認できない機能は停止を維持しました
...
| 対策 | 崩す要素 | 参照 |
| --- | --- | --- |
| 応答からの外部画像取得・リンクの自動プレビューを既定で禁止。例外的な取得も送信先・URL の内容・リダイレクト先を検証 | 外部送信(決定打の経路を封鎖) | [データ漏えい対策](../06-security/data-exfiltration.md) |
| 外部メールを読むフローと社内文書を検索するフローを分離し、両者の出力は非信頼データとして扱う。非公開情報を持つ処理には外部送信を許さない | 接触範囲の縮小と外部送信の制限。分離や要約だけを無害化の保証にしない | [シングルエージェントとマルチエージェント](../01-concepts/single-vs-multi-agent.md) |
...
画像プロキシは閲覧者の IP を隠せても、機微情報を含む URL を上流へ取得すれば、その path/query は攻撃者へ届きます。**プロキシ化だけでは持ち出し経路を閉じられません**。非公開メールを読む処理では外部取得を禁止し、必要な画像は検証済みの内部リソースなどへ限定します。
...
3. **実際の送信経路を先に止める** — 読み取りモードでも、応答表示が外部 URL を取得すれば漏えいは続きます。この事例では外部取得の遮断とセッション停止で封じ込め、その確認後に限定した要約機能を再開しました
...
- **出力レンダリングを攻撃面から除外する** → 画像・リンク経由の持ち出しを見逃す → 外部リソース取得を既定で禁止し、例外には送信先・URL 内容・リダイレクトの検証を適用する
- **読み取りモードやプロキシ化を封じ込め完了とする** → 履歴・要約対象の機密が URL に載って外部へ出る → 実際の外部通信を遮断し、進行中セッションを止めてから縮退再開を判断する
...
- [ ] 外部取得の遮断・進行中セッションの停止を、縮退再開より先に実行・検証できる
- [ ] プロキシ経由でも、機微情報を含む URL が外部へ送られない
...
- [Not what you've signed up for: Compromising Real-World LLM-Integrated Applications with Indirect Prompt Injection](https://arxiv.org/abs/2302.12173) — 取得データに埋め込む指示による API 操作・情報窃取を扱う原論文(アクセス日: 2026-09-10)
