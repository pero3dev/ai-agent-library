...
重要なのは、**効率化の施策の大半が[コスト管理](cost-management.md)と重なる**ことです。ただし作用は異なります。トークン削減・小さいモデルへの切替・キャッシュは、処理する計算量や重複計算を減らす施策です。バッチ処理は稼働率や処理効率を改善する余地がありますが、計算量の削減を一律に保証しません。低炭素の時間帯・リージョンへの移動は、同じ電力量でも排出を減らす施策です。料金の割引、電力消費、カーボン排出を同一視せず、ハードウェア効率、稼働率、電源構成、利用量を分けて測り、コスト最適化の環境効果を確認します。
...
- **データセンターの電力と冷却水**: 計算は電力を消費し、その多くが熱になり、冷却に電力と**水**を使います。施設の効率は PUE(電力使用効率 = 施設総電力 ÷ IT 機器電力、理想 1.0)、水は WUE(水使用効率、L/kWh)という指標で語られます。**PUE は IT 機器に対する施設の付帯設備の上乗せを示す比率で、IT 機器自体の計算効率や電源の炭素強度は別軸**です(低 PUE = 低排出ではない)
...
- **規模感は「帯」で捉える**: データセンターの電力需要や将来推計は、公的機関(IEA、米 LBNL/DOE など)が分析していますが、**将来はシナリオ幅が大きい**ため、単一の値を鵜呑みにしません。「無視できないが、前提次第で幅がある」という帯で捉えるのが誠実です。IEA の 2026-04-16 公開資料も参照し、タスク当たりの効率改善と、利用回数・タスク構成の変化による総需要を分けます

### エンジニアにできる削減

削減の主力は、特別なグリーン技術ではなく、**コスト最適化としてすでに確立された施策**です([コスト管理](cost-management.md)が正本)。

| 施策 | コストへの効果 | 環境への効果・条件 | 補足 |
| --- | --- | --- | --- |
...
| プロンプトキャッシュ | キャッシュ済み入力の割引 | 既に処理した共通の接頭部分(prefix)を再利用し、重複計算を減らす | 同じ prefix の再送自体は必要な場合がある([コスト管理](cost-management.md)のキャッシュ戦略) |
| 履歴圧縮 | 以後の送信トークンの削減 | 以後に処理する入力を減らす | 要約生成の計算と情報欠落を含めて評価する |
| セマンティックキャッシュ | モデル呼び出し回数の削減 | 呼ばない = 計算しない | 誤ヒット制御が前提([セマンティックキャッシュ](semantic-caching.md)) |
| バッチ処理 | バッチ API の割引 | 稼働率・処理効率を改善する余地。割引から計算量や電力の削減率は推定できない | 即時性不要の処理をまとめる([バッチ処理](batch-processing.md)) |
| 低炭素の時間帯への実行移動 | 契約・料金体系による | 同じ電力量でも炭素強度が低ければ排出を減らせる | オフピークが低炭素とは限らないため、実行時の電源構成を確認する |
...
要点は、**既存のコスト最適化施策から、計算量を減らせるものを特定すること**だという点です。right-sizing やキャッシュは計算量を減らす候補です。すべてのステップに最上位モデルを使わない設計([SLM 活用戦略](../03-implementation/slm-strategy.md))を品質評価と組み合わせ、総使用量と算定境界を揃えて削減効果を測ります。

### 測定の現実

自社の環境負荷を「正確に」測るのは難しく、限界を理解して使います。

- **推計手法**: 計算メタデータ(ハードウェア・稼働時間・リージョンの炭素強度)から排出を推計する手法(ML CO2 Impact Calculator 系)や、実行時に消費電力を実測するツール(CodeCarbon 系)があります。ソフトウェアの機能単位あたりの炭素強度を**レート**で算定する標準として Green Software Foundation の SCI(Software Carbon Intensity、ISO/IEC 21031)もあり、AI 向け拡張 SCI for AI は GSF が 2025-12 に批准した仕様です。学習・推論など AI ライフサイクルの算定境界と機能単位を明示します。基本 SCI の ISO/IEC 21031:2024 と GSF の AI 拡張は別であり、拡張まで ISO 規格化済みとは扱いません
...
- **ESG・サステナビリティ開示**: 企業の GHG インベントリは GHG プロトコル(スコープ 1/2/3)を事実上の基盤とし、EU の CSRD/ESRS E1 など制度開示がこれを参照します。EU 理事会は 2026-02-24 に簡素化法制を最終承認し、公式発表は対象企業を従業員 1,000 人超かつ純売上 4.5 億ユーロ超と説明しています。第三国企業・移行措置は別条件です。提案段階の説明だけで判断せず、改正法令と加盟国の国内法化・適用時期を照合し、**自社が対象かは法務・サステナビリティ部門と確認**します
- **義務報告と自主開示は別物**: 「自主開示(各社のサステナビリティレポート)」と「義務報告(EU EED のデータセンター報告など規制ベース)」は性質が違います。前者は PR を含み、後者は規制要求です。EED の公式 FAQ v1.8(本文版 2026-07、掲載 2026-07-28)は IT 設備の設置電力需要 500 kW 以上のデータセンターの年次報告を説明します。2026-09-10 の EC ページでは格付けパッケージは準備中であり、予定時期の経過だけで採択済みとは判断しません
...
- **「グリーン AI」を既存のコスト最適化と別プロジェクトにする** → 同じ「計算量を減らす」施策を二重に立てて労力を分散する → コスト施策のうち計算量を減らすものを特定し、[コスト管理](cost-management.md)に環境の観点を足す
...
- [European Commission — Energy performance of data centres](https://energy.ec.europa.eu/topics/energy-efficiency/energy-efficiency-targets-directive-and-rules/energy-efficiency-directive/energy-performance-data-centres_en) — EED に基づくデータセンター報告・FAQ v1.8・格付け準備状況(アクセス日: 2026-09-10)
...
- [GSF — SCI for AI](https://greensoftware.foundation/standards/sci-ai/) — AI ライフサイクルの境界を扱う GSF 仕様(アクセス日: 2026-09-10)
- [AWS Sustainability console GA](https://aws.amazon.com/about-aws/whats-new/2026/03/aws-launches-sustainability-console/) / [利用ガイド](https://docs.aws.amazon.com/sustainability/latest/userguide/getting-started.html) — 後継ツール・取得経路・収録範囲(アクセス日: 2026-09-10)
- [IEA — Key Questions on Energy and AI](https://www.iea.org/reports/key-questions-on-energy-and-ai) — 2026-04-16 公開の分析(アクセス日: 2026-09-10)
- [EU 理事会 — サステナビリティ報告等の簡素化の最終承認](https://www.consilium.europa.eu/en/press/press-releases/2026/02/24/council-signs-off-simplification-of-sustainability-reporting-and-due-diligence-requirements-to-boost-eu-competitiveness/) — 2026-02-24 の承認と範囲の説明(アクセス日: 2026-09-10)
- [Google 2026 Environmental Report](https://sustainability.google/google-2026-environmental-report/) / [Microsoft 環境報告](https://www.microsoft.com/en-us/corporate-responsibility/topics/sustainability/report/) / [Amazon 報告一覧](https://sustainability.aboutamazon.com/reports) — 公表版と実績期間を別々に確認(アクセス日: 2026-09-10)
