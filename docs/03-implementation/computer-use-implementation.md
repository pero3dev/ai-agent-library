...
```mermaid
flowchart TD
    S["スクリーンショット取得<br/>(観測)"] --> D["モデルが行動案を作成<br/>(対象・内容の特定)"]
    D --> P{"実行権限・許可範囲内か?"}
    P -->|No| X["実行せず停止・差し戻し"]
    P -->|Yes| G{"不可逆操作など<br/>承認が必要か?"}
    G -->|Yes| H{"対象・内容を示して<br/>人間に承認を求める"}
    H -->|却下・期限切れ| X
    H -->|承認| A["許可された行動の実行<br/>(クリック・入力・キー送信)"]
    G -->|No| A
    A --> W["待機<br/>(描画・遷移の完了)"]
...
- **観測 → 行動案 → 権限確認・必要時の承認 → 実行 → 待機 → 検証**を 1 周とします。承認は副作用の発生前に行い、承認後に対象・送信内容・画面状態が変わった場合は再観測して承認を取り直します。行動のたびに画面を撮り直すため、不要な周回を減らす設計がレイテンシと画像処理コストを抑えます
...
- **API がある操作を画面操作で実装する** → 座標・画面変更への依存と誤クリックのリスクが増える → 手順を棚卸しし、API・MCP で置換できる区間を先に切り出す。API のツール結果にもインジェクションは入りうるため、認可・送信前承認は維持する
...
- [ ] 対象手順を棚卸しし、API・MCP で置換して GUI に残す区間を最小化した
...
- [ ] 不可逆操作の前に承認ゲートがあり、却下・期限切れでは実行されず、承認対象の変更時は再承認になる
- [ ] 緊急停止手段がある
...
- [RPA と Agent の使い分け・移行戦略](../13-domain-agents/rpa-and-agents.md) — 画面操作の RPA 資産との共存・移行(computer use を移行先の一手段として位置づける)
