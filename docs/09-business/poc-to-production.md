...
有効な中間手段として **シャドーラン**(本番の入力を Agent に流すが、結果は業務に使わず人の結果と突き合わせる)があります。**送信・購入・DB 更新などの副作用を持つツールは無効化またはモック化し、応答も利用者に配信しない**ことが条件です。最終回答だけを捨てても、途中のツール実行による業務への影響は消えません。読み取り先への負荷、推論費、データの複製・保持にも上限と管理を設けます。この条件下で本番分布の品質を測り、関門 1 と関門 2 の間に挟みます([バージョニング・デプロイ・モデル更新追従](../05-operations/versioning-and-model-updates.md))。
...
- [AI 時代のチームトポロジー](ai-team-topologies.md) — 本番化を支える体制・役割配置の設計
- [AI 調達・ベンダー選定の実務](ai-procurement.md) — 「作る」でなく「買う」ときの PoC 条件設計・検収・継続ガバナンス
