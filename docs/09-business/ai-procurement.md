...
AI ソリューションでも、従来の SI・SaaS 調達で用いる要件定義・受け入れ試験・契約管理は有効です。生成品質のばらつきを扱うため、次の 3 点を補強します。

1. **振る舞いの制約と品質評価を分ける**: 「こう入力したらこう出力する」を網羅的に規定できません。出力は確率的で、同じ入力でも揺れます。権限・形式・処理上限などは仕様で規定し、開放的な回答品質は代表データと失敗分類で評価します
2. **性能が確率的**: 「95% 正しい」システムを、残り 5% を織り込んで調達する必要があります。決定的システムの「バグかどうか」の二分法が効きません
3. **デモが当てにならない**: ベンダーのデモは、うまくいく入力で最適化されています。**自社の実データ・エッジケースへの適合は、選ばれたデモだけでは判断できません**([ケーススタディ: 撤退した PoC](../07-case-studies/case-study-failed-poc.md)の「デモの罠」)
...
- **SLA(サービス品質保証)**: 可用性だけでなく、品質(精度)の保証をどう扱うか。検証なしに「精度 100%」を前提にせず、測定方法・許容範囲・下回った場合の扱いを論点にします
...
**仕様上の制約を満たすか**と、**受け入れ評価セットで品質水準を満たすか**の両方で検収します。
...
AI ソリューションは、ベンダー側のモデル更新で**買った後に挙動が変わり得ます**。SaaS の更新管理に加え、生成品質の変化を検出する仕組みが必要です。
...
- **測定範囲を決めず「精度 100%」の SLA を求める** → 達成判定や違反時の対応が曖昧になる → 対象・測定方法・評価期間・例外・目標値と、未達時の対応を契約に定義する
...
- [NIST AI RMF 1.0 — Core](https://airc.nist.gov/airmf-resources/airmf/5-sec-core/) — GOVERN 6・MAP・MEASURE による調達先リスク、利用文脈、測定の整理。RFP・検収手順は本記事の応用です(アクセス日: 2026-09-10)
