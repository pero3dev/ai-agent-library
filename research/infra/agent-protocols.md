...
- **調査 ID**: IF-R1
- **調査日 / 確認日**: 2026-07-09(初回)/ **定点観測**: 2026-08-18(四半期。A2A の AAIF 正式編入・AP2 の FIDO Alliance への所有権寄贈・MCP 現行リビジョン 2026-07-28・AGNTCY の LF 寄贈一次確認を反映)
...
| **A2A は MCP と同じく Agentic AI Foundation(AAIF)傘下へ正式編入済み**。AAIF の project-proposals リポジトリで提案 Issue **#37** が承認され、aaif.io/projects のホストプロジェクト一覧に Agent2Agent(A2A)が掲載されている(2026-07-09 時点の「未確認」項目が解消) | https://github.com/aaif/project-proposals/issues/37 / https://aaif.io/projects | 2026-08-18 | 公式確認済み |

**仕様が定める要素(現行バージョン)**

| 事実 | 出典 URL | 確認日 | 確度 |
| --- | --- | --- | --- |
| 現行プロトコルバージョンは **`1.0.0`**(初の安定版)。リクエストヘッダに `A2A-Version`、拡張に `A2A-Extensions`(2026-08-18 再確認: 仕様の表記は `1.0.0` のまま。GitHub リリースの最新は **v1.0.1**〔2026-05-28、バグ修正のみのパッチ〕) | https://a2a-protocol.org/latest/specification/ / https://github.com/a2aproject/A2A/releases | 2026-08-18 | 公式確認済み |
...
> 認可仕様(RS/AS 分離、CIMD、step-up 等)の詳細は [agent-identity.md](../professional/agent-identity.md) §2 に整理済み(現行 2026-07-28 リビジョンの認可差分は同 §2.6)。ここでは相互運用・ガバナンスの現在地のみ。
...
| 2026-08-18 時点の AAIF ホストプロジェクトは **5 つ: MCP / goose / AGENTS.md / agentgateway / A2A**(発足時の 3 つから agentgateway・A2A が追加)。ワーキンググループは **8 つ**: Identity & Trust、Agentic Commerce、Accuracy & Reliability、Governance, Risk & Regulatory Alignment、Observability & Traceability、Security & Privacy、Workflows & Process Integration、Taxonomy & Landscape | https://aaif.io/projects / https://aaif.io/ | 2026-08-18 | 公式確認済み |
| MCP 仕様の現行(Current)リビジョンが **2026-07-28** に更新(破壊的変更を含む大改版): **ステートレス化**(`initialize` ハンドシェイク廃止・プロトコルバージョンは `_meta` で搬送・`server/discover` 必須化)、**セッション(`Mcp-Session-Id`)の削除**、**tasks の公式拡張化**、**Roots / Sampling / Logging の Deprecated 化**。認可章の差分(DCR の Deprecated 化・RFC 9207 `iss` 検証追加)は [agent-identity.md](../professional/agent-identity.md) §2.6 参照 | https://modelcontextprotocol.io/specification/2026-07-28/changelog | 2026-08-18 | 公式確認済み |
...
| **2026-04-28、Google が AP2 の所有権(ownership)を FIDO Alliance へ正式寄贈**(v0.2 リリースと同時)。FIDO は Agentic Authentication Technical WG を組成し、agent-initiated commerce の相互運用標準を策定へ。v0.2 は「**Human Not Present**」(事前承認済みのユーザー指示に基づく自律決済)を導入。**Mastercard と共同開発の Verifiable Intent フレームワークも併せて FIDO へ寄贈** | https://fidoalliance.org/fido-alliance-to-develop-standards-for-trusted-ai-agent-interactions/ / https://blog.google/products-and-platforms/platforms/google-pay/agent-payments-protocol-fido-alliance/ | 2026-08-18 | 公式確認済み |
...
| **AGNTCY(Internet of Agents)**: Cisco が 2025-03 にオープンソース化(LangChain・Galileo と協働)し、**2025-07-29 に Linux Foundation へ寄贈**(LF 公式プレスで一次確認。2026-07-09 時点の二次情報から確度昇格)。Cisco・Dell・Google Cloud・Oracle・Red Hat が formative members。**Agent Directory Service**(能力・メタデータ・検証可能レコードで DNS 的に発見)+ Agent Connect Protocol 等の「stack」 | https://www.linuxfoundation.org/press/linux-foundation-welcomes-the-agntcy-project-to-standardize-open-multi-agent-system-infrastructure-and-break-down-ai-agent-silos | 2026-08-18 | 公式確認済み |
...
1. **A2A の AAIF 傘下入りの有無**: **解消(2026-08-18)**。project-proposals Issue #37 の承認と aaif.io/projects への掲載で正式編入を一次確認。今後は AAIF 内でのガバナンス・TSC 構成の変化を追う
2. **A2A の仕様バージョン**: 現行の仕様表記 `1.0.0`(2026-08-18 再確認。GitHub リリース最新は v1.0.1〔2026-05-28・バグ修正〕)。マイナー/拡張(`A2A-Extensions`)の追加が速い。仕様参照は必ず `a2a-protocol.org/latest/specification/`(または版固定 URL)で確認し、Task 状態・discovery 方式の変更を追う
3. **A2A のレジストリ API**: discovery のうちレジストリ方式は「標準 API 未策定」(2026-08-18 再確認: 未策定のまま)。標準化されたら記事の「発見」節を更新
4. **MCP のリビジョン**: 現行 `2026-07-28`(2026-08-18 確認。破壊的変更を含む大改版: ステートレス化・セッション削除・tasks 公式拡張化・Roots / Sampling / Logging の Deprecated 化)。認可章は毎リビジョンで大きく変わる実績(詳細は agent-identity.md §2.6)。AAIF 移管後の SEP プロセスの動きも追う
5. **AAIF の構成**: 発足 2025-12-09。2026-08-18 時点でホストプロジェクト 5(MCP / goose / AGENTS.md / agentgateway / A2A)・ワーキンググループ 8(Identity & Trust、Agentic Commerce 等)。Platinum メンバー・プロジェクト・WG は拡張が速い(x402 等の編入可能性)
6. **AP2 の mandate モデルとバージョン**: 発表時 V0.1(Intent / Cart / Payment mandate)→ V0.2(Checkout / Payment mandate、VDC、「Human Not Present」)。**2026-04-28 に所有権が Google から FIDO Alliance へ寄贈済み**(Mastercard 共同の Verifiable Intent も寄贈)。以後の改訂は FIDO 側(Agentic Authentication / Payments Technical WG)で追う
7. **ACP / AGNTCY の収斂**: IBM ACP は A2A へ統合済み(2025-08)。AGNTCY は LF 寄贈(2025-07-29)を LF 公式プレスで一次確認済み(2026-08-18)。A2A との関係・スタックの位置づけは引き続き流動的で、AGNTCY 公式サイトの動きを追う
...
### 定点観測(2026-08-18)で追加・確認した URL(アクセス日 2026-08-18)
- https://aaif.io/projects(AAIF ホストプロジェクト 5 つ: MCP / goose / AGENTS.md / agentgateway / A2A)
- https://aaif.io/(AAIF ワーキンググループ 8 つ)
- https://github.com/aaif/project-proposals/issues/37(A2A の AAIF プロジェクト提案・承認)
- https://github.com/a2aproject/A2A/releases(最新リリース v1.0.1・2026-05-28)
- https://modelcontextprotocol.io/specification/2026-07-28/changelog(MCP 2026-07-28 リビジョンの変更点)
- https://fidoalliance.org/fido-alliance-to-develop-standards-for-trusted-ai-agent-interactions/(FIDO の AP2 受け入れ・Agentic Authentication WG・2026-04-28)
- https://blog.google/products-and-platforms/platforms/google-pay/agent-payments-protocol-fido-alliance/(Google の AP2 所有権寄贈・Verifiable Intent)
- https://www.linuxfoundation.org/press/linux-foundation-welcomes-the-agntcy-project-to-standardize-open-multi-agent-system-infrastructure-and-break-down-ai-agent-silos(AGNTCY の LF 寄贈・2025-07-29 プレス)
