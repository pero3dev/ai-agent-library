...
### 概要: 実行の状態を API の契約にする

Agent を API として公開するときは、即時に結果を返す CRUD API に加えて、実行状態を表すリソースが必要になる場合があります。HTTP・REST の設計で表現できますが、次の性質を契約に織り込みます。
...
- **投入**: `POST /jobs` がジョブを受理し、ジョブ ID と状態照会先を返す(HTTP 202)。202 は受付済み・処理未完了を示し、最終的な成功を保証しません([RFC 9110 §15.3.3](https://www.rfc-editor.org/rfc/rfc9110.html#section-15.3.3))
...
- **スコープと保持期間**: キーの一意性の範囲(例: テナントと業務操作)と、何時間・何日まで同一視するか。認証用 API キーの更新で同じ操作の重複排除が失われない設計にします
- **ペイロード不一致の扱い**: 同じキーで異なる内容が来たらエラーにする(サイレントに片方を無視しない)
- **どの操作に必須か**: 少なくともジョブ投入と、承認などの副作用を持つ操作すべて

API 層では、キーの登録とジョブ作成を一意制約・トランザクション等で原子的に行い、同時再送でもジョブが増えないようにします。内部の重複実行対策([非同期・長時間タスクの設計(耐久実行)](async-and-durable-agents.md)の冪等性)とは層が異なります。ワーカーの再試行やジョブ再作成でも同じ業務操作を二重反映しないよう、外部の副作用先まで安定した業務キーを引き継ぎます。受信側の重複排除契約がない場合、応答不明の操作は自動再送せず、結果を照合してから再実行可否を決めます。
