...
- **冪等性**: 副作用のあるツールには、受信側が保証する冪等キー契約、または同一 DB 内の重複排除と更新の原子性を設けます。実行済みかの事前チェックだけでは、ワーカー間の競合や外部での成功後・記録前の停止を防げません。冪等性に非対応の外部 API で結果不明になった場合は、自動再送を止めて結果を照合します([エラー処理・リトライ設計](../02-architecture/error-handling-and-retries.md)、[耐久実行](../02-architecture/async-and-durable-agents.md))
...
- [ ] 副作用の受信側の冪等契約または同一 DB 内の原子性があり、結果不明時は自動再送を止めて照合する
...
- [LLM ゲートウェイの設計](llm-gateway.md) — フォールバック・ルーティングを 1 つの部品に集約する実装点
- [セルフホスト推論の実務](self-hosted-inference.md) — モデル提供層そのものを自前で持つ場合の容量・切り替え

## 参考資料

- [Making retries safe with idempotent APIs(AWS Builders' Library)](https://aws.amazon.com/builders-library/making-retries-safe-with-idempotent-APIs/) — 重複排除と副作用を原子的に確定する設計(アクセス日: 2026-09-10)
