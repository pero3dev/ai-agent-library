...
開始日: 2026-09-24。状態: **P0のPR検証・表示通知の修正中**。全199記事の完了記録ではない。
...
| P0 | 共通基盤・自己注意・Agentループ・Workflow比較 | PR #52検証・表示通知の修正中 | [P0実施記録](reading-diagram-foundation.md) |
...
P0の3図を共通外枠へ統合し、独立レビュー・ローカル受入後に [PR #52](https://github.com/pero3dev/ai-agent-library/pull/52) を作成した。CIで判明したIntersectionObserverの複数通知の取りこぼしを再現し、Mermaidの起動と共通図解の可視状態を修正中。次の操作は修正後の再ビルド・回帰・独立レビュー・CI・公開確認を進めること。

P0受入後は [P1の最初の3記事の絵コンテ](llm-diagram-storyboards.md)を起点にする。最初の制作単位は、数値・構造が複雑なTransformer1記事（追加3図と既存自己注意）とする。注意変種3図・MoE2図を後続で進め、全31見出しの論点対応を用いて各記事全体の受入を判定する。この絵コンテは準備段階で、P1の実装・公開は未着手。
