...
- 調査日: 2026-07-07(2026-08-18 に定点観測で主要項目を再確認 — 差分は各節に追記。WebArena・AndroidWorld・BFCL・GAIA 無印・MLE-bench・安全性系は 2026-08 未再確認)
...
- https://www.tbench.ai/
- https://github.com/ServiceNow/webarena-verified/blob/main/README.md
...
| Pro(Scale AI) | エンタープライズ級の長期タスク。全 1,865 問 = public 731(GPL リポジトリ)+ commercial 276(非公開スタートアップコード)+ held-out 858(結果非公開)。汚染耐性を明示的に設計。2026-07 時点の公式トップ帯は **5 割台** → 2026-08-18 再確認で **6 割台前半**(Muse Spark 1.1 61.5±3.1 / gpt-5.4 xHigh 59.1)。3 セット構成の運用は継続 | https://labs.scale.com/leaderboard/swe_bench_pro_public / 2026-08-18 / 公式確認済み(論文 arXiv 2509.16941 は二次情報) |
...
- **(2026-08-18 追記)Terminal-Bench 3.0 が 2026-07-23 に公開**(旧称 FrontierBench・Harbor 上・v0.1 は 74 タスク / 7 領域・Scale AI が最多タスク貢献)。公式アナウンスが「TB 2.x の多くのタスクは飽和」と明言。公開時点の最前線トップは約 34%(GPT-5.6 Sol 34.4%)。2.x の置き換えではなく拡張で、2.1 リーダーボードも提出受付継続(2026-07-09 提出エントリあり・トップは Claude Code + Fable 5 の 83.8%±1.2)。※二次集約サイトの「89.5%」は公式ボードと不一致(自己報告系の可能性・不採用 — 自己報告と実測の乖離の好例)。周辺: **Harbor-Index**(軽量エージェント評価・2026-07-06 発表)、**Terminal-Bench Science**(自然科学ワークフロー・2026-05-20 発表・開発中)|https://www.frontierbench.ai/announcement ・ https://labs.scale.com/blog/terminal-bench-harder-tasks-for-better-agents ・ https://www.tbench.ai/leaderboard/terminal-bench/2.1 ・ https://www.tbench.ai/news / 2026-08-18 / 公式確認済み
...
| OSWorld | 実 OS 上のコンピュータ操作 369 タスク・実行ベース評価。人間 72.36%。**OSWorld-Verified**(2025-07): 約 300 件の問題報告を 10 人 × 2 か月で修正。リーダーボード掲載はメンテナーがコードを実行する第三者実測枠あり。トップは 80% 台(二次)で人間超え → **OSWorld 2.0**(2026-06): 108 の長時間ワークフロー(平均 318 ツール呼び出し)、ベスト 20.6%(2026-08-18 再確認: 20.6% 据え置き・Claude Opus 4.8。v1 サイトに 2026-06-26 の 2.0 公開告知) | https://osworld-v1.xlang.ai/ ・ https://xlang.ai/blog/osworld-verified ・ https://osworld-v2.xlang.ai/ / 2026-08-18 / 公式確認済み(2.0 スコアも公式) |
...
| HLE(Humanity's Last Exam) | 2,500 問・100 超科目・約 1,000 名の専門家作問(CAIS + Scale AI、Nature 掲載)。**非公開ホールドアウトセット**を保持。公式サイト表示トップ 38.3%(最新モデルへの追随遅れの可能性)、第三者計測は評価条件により 45〜65% 帯。ツール・検索併用で +6〜12pt 上振れ。2026-08-18 再確認: 表示トップ 38.3% 据え置き(現表示は Gemini 3 Pro)。バグバウンティでフラグされた問題を差し替えた動的フォーク版 **HLE-Rolling**(2025-10-08 公開)あり | https://lastexam.ai/ ・ https://arxiv.org/abs/2501.14249 / 2026-08-18 / 公式確認済み(第三者帯・ツール上振れは二次/ベンダー自己報告) |
...
| τ-bench(Sierra) | ユーザーシミュレータとの対話 + ドメイン API + ポリシー遵守。会話終了時の DB 状態比較で自動判定。**pass^k**(k 回全試行成功)で信頼性を測定 — retail の pass^8 < 25%。airline 中心にタスク不備があり、**τ³-bench**(2026-03、banking 追加・音声評価対応)で airline 27 + retail 26 タスクを修正。修正で airline pass^1 が +14〜20pt 変動 = 旧スコアは過小評価だった。2026-08-18 再確認の現行トップ帯: τ³-Banking 55.2%(Qwen 3.8 Max)/ τ³-Voice 67.3% / τ²-text 87.9%(8 割台後半で飽和接近)。2026-02 に retail/airline 50+ タスクの監査・修正完了、2026-03 にリアルタイム音声評価を開始 | https://arxiv.org/abs/2406.12045 ・ https://github.com/sierra-research/tau2-bench ・ https://taubench.com/ / 2026-08-18 / 公式確認済み |
...
| サイト | 性格 | 出典 / 確認日 / 確度 |
| --- | --- | --- |
| HAL(Holistic Agent Leaderboard、Princeton) | **第三者による標準化実測**。統一ハーネスで全エージェントを再実行しログ全公開。**コスト × スコアのパレート表示**が中核("Agents can be 100x more expensive while only being 1% better")。9 ベンチ × 9 モデル・21,730 実行の論文(arXiv 2510.11977)では「推論努力を上げると過半の実行で精度が低下」も報告。2026-08-18 再確認: **新モデルでのリーダーボード更新を一時停止し、エージェントの信頼性測定に注力**と公式明記(対象 9 ベンチのまま)。**CORE-Bench は "solved" を宣言**(Opus 4.5 + Claude Code スキャフォールドで大幅超過)。HAL 論文は ICLR 2026 採択 | https://hal.cs.princeton.edu/ / 2026-08-18 / 公式確認済み |
...
- (2026-08-18 再試行)openai.com の評価停止告知 403 継続 / GAIA2 リーダーボード(HF Space)JS レンダリングで数値取得不可継続(公式ブログ最新更新は 2025-10-02)/ swebench.com スコア表の途中切断継続
...
4. 定点観測(四半期): Terminal-Bench 3.0 のスコア推移と 2.1 との並存 / SWE-bench Pro トップ帯 / HAL の新モデル追加再開の有無 / GAIA2 の一次数値 / OSWorld 2.0 / τ³。次回は 2026-08 に未再確認だった WebArena・AndroidWorld・BFCL・GAIA 無印・MLE-bench・安全性系も一巡させる
